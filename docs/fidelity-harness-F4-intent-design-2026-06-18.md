...
**Correction (matches the shipped code, 2026-06-19):** `BrainAdapter.gather_vault` calls `context_lib.gather(handle, sources=["vault"])` **DIRECTLY** — NOT via `server.gather_context`. So it never benefits from the `server.py:209-212` `ts.isoformat()` conversion: each raw `context_lib` vault hit carries a **`datetime`** `ts` (mtime-derived — `context_lib.py:144-147`, `datetime.fromtimestamp(mtime, tz=utc)`) keyed under **`ref`** (the path), `text`, `heading`. That `datetime`/mtime `ts` is the **wrong clock and untrusted**: `_content_ts` only accepts a *string* `ts` (`isinstance(ts, str)`), so the datetime is rejected and the content date is derived from the **path** instead. Consequently `filter_mcp_result` is **belt-only on vault hits** (its `_item_ts` requires a string ISO `ts`, which the raw hit lacks), and the **LOAD-BEARING fence is `_content_ts`'s path-date extraction**, which deliberately **OVER-EXCLUDES**: only path-dated content is admitted; an mtime-only hit (no derivable path/heading date) is dropped. The `brief` is never read. The `0-Self/` privacy fence still holds on this path: `0-Self/` is **excluded from the embeddings index itself** (`server.py:154-156`), so `lib.search` (which `context_lib._fetch_vault` calls) never returns a `0-Self/` hit in the first place; `read_note` is additionally blocked from `0-Self/` by `_validate_read_path`. The cutoff fence holds either way.

### 2.3 `_content_ts(hit)` — the content-clock helper (B3 detail) — THE LOAD-BEARING FENCE
`mtime` (file edit) is the wrong clock for the cutoff. Because `gather_vault` calls `context_lib.gather` **directly** (§2.2 correction), the raw vault hit's `ts` is a **`datetime`** (mtime-derived), not an ISO string — so `filter_mcp_result` cannot fence vault hits (its `_item_ts` only matches string ISO `ts`), and `_content_ts` is the **single load-bearing fence** on the vault tier. It derives a conservative content ts for a vault hit from, in order: (a) a *string* ISO `ts` field — present only if a caller passes one (the raw `context_lib` hit does NOT, so this branch is effectively dormant on the live path and exists for forward-compat with a future content-ts index); falling back to (b) a date parsed from the note path / daily-note name (`1-Daily/2026-05-12.md`, keyed under `ref`/`path`/`heading`); and **if neither yields a content date, the hit is excluded** (treated as un-fenceable). This is a deliberate **OVER-EXCLUSION**: a `datetime` (mtime) `ts` is rejected by the `isinstance(ts, str)` guard and **never** admits a hit. There is **no `mtime` fallback — ever**. The live consequence: on today's index, only path-dated notes (daily notes, dated files) are admissible vault content — the rich body of undated `3-People/` and `5-Reflections/` notes is excluded until §10 item 4 ships.
...
> is the keystone enabler of rich intent-fidelity** (re-admits the richest
> retrieval tool AND lets `_content_ts` fence the full undated vault corpus);
> until it ships, the measured context-lift is a FLOOR (see item 4). **(5)**
...
4. **Search-index content-timestamps (§2.3, Blocker 3) — THE KEYSTONE ENABLER OF RICH INTENT-FIDELITY.** `search_brain` is excluded from eval-mode gathering, and the direct `context_lib.gather` vault tier admits **only path-dated notes** (the `datetime`/mtime `ts` is the wrong clock — §2.2/§2.3 — so undated `3-People/`, `5-Reflections/`, `9-Codebases/` content is over-excluded). This is the single biggest limiter on how much real as-of-cutoff substance the officer-under-test can see. **Adding a per-chunk *content* timestamp to the brain index is therefore not a mere scheduled follow-up — it is the keystone enabler of rich intent-fidelity.** It re-admits the single richest retrieval tool (`search_brain`) AND lets `_content_ts` fence the full undated vault corpus by a trustworthy clock instead of dropping it. **Until it exists, gathering admits only: path-dated notes + fenced commitments + static person frontmatter + the pre-cutoff thread (+ explicit pre-cutoff `read_note` paths).** Therefore the context-lift the A/B harness measures with today's gathering is a **FLOOR, not the ceiling** — the true intent-fidelity headroom is gated on this index change, and reported numbers should be read as a lower bound. (F4 ships the `read_note`-on-pre-cutoff-paths fallback as the interim; this item is the unlock.)
