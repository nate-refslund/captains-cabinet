...
**Germline (Ground 1 + `[FIX-8]`).** `framework/policies/*` (covers `authority-matrix.yml`) and the exact file `cabinet/scripts/lib/policy_engine.py` are already germline in `pre-tool-use.sh` Section 5. The new judge modules (`authority_thermostat.py`, `authority_veto.py`, `deploy_classifier.py`, `action_classifier.py`, `lane_resolver.py`) are added to the germline case **by name in the same commit that creates them** — see §8.
...
- **Germline read-only** (`cabinet/tests/hook-regression/germline-readonly.sh`): officers can't edit `authority-matrix.yml`, `policy_engine.py`, **or any new judge module** (`authority_thermostat.py`, `authority_veto.py`, `deploy_classifier.py`, `action_classifier.py`, `lane_resolver.py`) `[FIX-8]`.
...
- New artifacts proposed:
  ...
  - Co-located judge modules in `cabinet/scripts/lib/`: `action_classifier.py` (shared `classify_action`), `lane_resolver.py` (`resolve_lane`), `authority_thermostat.py`, `authority_veto.py`, `deploy_classifier.py` — **all germline-registered** in `/Users/nate/captains-cabinet/cabinet/scripts/hooks/pre-tool-use.sh` Section 5 `[FIX-8]`.
  ...
  - `CABINET_LANE` export in `/Users/nate/captains-cabinet/cabinet/scripts/start-officer.sh` `[FIX-4]`.
