<!-- CLAUDE-egg.md — the CAPTAIN-AGNOSTIC operating context that SHIPS AS
     CLAUDE.md in the public egg (swapped in at egg export by the
     `claude-egg-swap` transform; PC-E scrub-paths 2026-07-10).
     The repo-root CLAUDE.md in a LIVE checkout is officer-loaded deployment
     context and may carry that deployment's concrete values; THIS file must
     stay generic — zero captain names, zero absolute paths, zero real
     product/org values. Examples use the synthetic testburg estate
     (captain Ada, bakery/newsletter lanes, example.com). -->
...
- **`presets/<active>/`** — use-case configuration (active preset in `instance/config/active-preset`, default `work`). Adds agent archetypes, terminology, constitution/safety addenda, additional schemas. Core presets: `work` (five functional officers, single product), `portfolio` (one persistent Chair + on-demand per-lane CEOs — recommended for multi-product captains), `personal`, plus `_template` scaffolding for authoring your own.
...
**Cabinet axes — data, never branches.** The deployment is configured along three orthogonal axes — `autonomy_level` (earn_up | guardian | sovereign), `flavor` (personal | org), `deployment_target` (macbook | mac_mini | docker) — consumed ONLY through the sanctioned resolvers and tables (`framework/authority/posture.py` → the authority matrix); never write `if posture == ...` or any axis comparison in code (CI's axis linter rejects it outside the germline allowlist). The full contract, including extension rules and the upgrade-is-attested / downgrade-is-always-allowed asymmetry, is `.claude/rules/axes-contract.md` (germline; spec of record `docs/plans/cabinet-axes-spec-2026-07-05.md` §6).

**Framework code is launcher-agnostic — address the captain via the resolver, never a hardcoded name.** `framework/` is the universal base for any captain and either flavor. Framework code that greets or represents the captain in a runtime string (prompt, digest, briefing, message) calls `framework.env.captain_name()` (reads `instance/config/platform.yml`, fallback `"Captain"`), and finds the repo root via `CABINET_ROOT` / a file-relative `parents[N]` — never a captain-name literal or an absolute home path. The captain's name, vault paths, board ids, and colleague names live in `instance/`, never in `framework/`. The clean-room ratchet (`framework/tests/test_no_launcher_hardcode.py`) enforces it in CI: a launcher-specific name or home path in `framework/**/*.py` outside its documented shrink-only allowlist is CI-red, not a review note (sister ratchet to the axis linter).

**Instance-specific DATA and Flavor-A code live in `instance/`, reached through `framework.env` resolvers — `framework/` hardcodes no launcher's data.** The same discipline that lifts the captain's NAME to `framework.env.captain_name()` lifts every other launcher-specific datum: the captain's ROLE/title via `framework.env.captain_role()` (fallback `"the Captain"`), the org's internal email domains via `framework.env.org_domains()` (fallback `()` — every recipient classifies external, the conservative comms ceiling), and the tasks-board id via `framework.env.tasks_board()` (env `CABINET_TASKS_BOARD` overrides; fallback `""`, whereupon the executor's `isdigit()` guard refuses rather than leaking a board). All read `instance/config/platform.yml` (else `product.yml`), cached once per process, **fail-closed to a generic default** — a fresh deployment inherits none of the first instance's org, board, or role. The lane/officer family follows the same discipline: the officer roster via `framework.env.officers()` (officer column of `cabinet/officer-capabilities.conf`, conf file order; fallback `()` — a rosterless deployment rejects every delegate target, never a baked-in roster), the eval probe officer via `framework.env.deploys_code_officer()` (first `deploys_code` holder; fallback `""`), the instance lane enum via `framework.env.lanes()` (sorted `slug:` scalars from `instance/config/contexts/*.yml`, deliberately NOT filtered on `active:`; fallback `()`), and the acting lane's proposal default via `framework.env.lane_default()` (fallback `""` — cards file under the stable `adhoc` catch-all). Launcher-specific Flavor-A code lives under `instance/flavor-a/`, and `framework/` must never import it (the layer-separation gate, `cabinet/scripts/check-layer-separation.sh`, enforces the one-way `framework/ → instance/` boundary). The captain's personal *sensing estate* (vault / brain: person intel, commitments, voice) is reached from framework CORE ONLY through `framework.sources.get_source()` (the personal-sensing seam) and `get_dispatch()` (the write seam) — implementations are instance-bound adapters declared in `instance/config/sources.yml`, and a deployment with none bound gets `NullPersonalSource` (fail-closed empties). A personal-tooling import or home-path anywhere in `framework/**` is CI-red (`framework/tests/test_no_screenpipe_in_core.py`).
...
- **Business context:** where the lane's config points it — Notion (`notion-search` / `notion-fetch`) when the lane carries Notion IDs; a lane may deliberately keep knowledge elsewhere (e.g. a docs system or knowledge vault, per its `projects/<lane>.yml`)

Do not hallucinate product knowledge — discover it from artifacts.

## Addressing the Captain

Read `captain_name` from `instance/config/platform.yml` (single-product deployments may carry it as `product.captain_name` in `instance/config/product.yml`). When speaking to or about the Captain in messages, briefings, and voice — use their name (e.g. "Ada" not "Captain"). If `captain_name` is not set, fall back to "Captain."
...
| **GitHub Issues** | Cabinet-framework backlog — infra, officer system, meta-features | `gh` CLI / GitHub API on the framework repo's `origin` remote |
| **Git repo** | Code — the product itself | Git CLI in the lane's product checkout (path in `instance/config/projects/<lane>.yml`) |

Keep framework work (GitHub Issues) separate from product work (/tasks) so the product officer never triages framework items.

## Notion Usage

Officers read from and write to Notion **where the lane configures it**. Key locations (IDs in the lane's `instance/config/projects/<lane>.yml` → `notion:` block, or `instance/config/product.yml` on single-product deployments; a lane may deliberately leave them empty):
...
- **Log every Captain decision with its WHY**, the moment it's made — to `shared/interfaces/captain-decisions.md` via `cabinet/scripts/append-interface.sh captain-decisions` with the entry on stdin (the three captain-law ledgers are append-only through that interface; on the Mac runtime, officers are kernel-blocked from the files and the fixed-policy broker stamps every submitted observation as `[trust:officer]`) (+ the `captain-decision` gold label & comment on affected issues). Read the trail before any design/UI/feature work; never re-introduce something the Captain killed. Officers with `logs_captain_decisions` log in real time (post-reply hook enforces); CoS syncs the summary during briefings.
...
- **4th loop — Pattern Listening (reactive).** The three self-improvement loops cycle slower than in-conversation signals arrive, so listen inline: scan each Captain DM for meta-signals (process questions, memory/tracking hints, "always/never" preferences, implicit frustration, repeated phrasings); on a hit, make a one-sentence encode-offer (or just encode on the 2nd occurrence per the two-count rule), then encode it via the sanctioned append interface — `cabinet/scripts/append-interface.sh captain-patterns` with the entry on stdin (heredoc/pipe; append-only, provenance-stamped as `### officer-note … [trust:officer]`) — and broadcast. On Mac, the officer sandbox prevents direct mutation even through dynamically constructed paths; the broker owns identity/provenance and always rejects Captain-format headings. Officer text is an observation, never self-ratified standing law. Full mechanics: `memory/skills/captain-pattern-listening.md`.
...
All research briefs are embedded and stored in PostgreSQL via pgvector. This makes research persistent, searchable, and reusable across restarts.
...
- Before committing infrastructure changes, writing a major spec, shipping a research brief, or making a significant decision: spawn a review agent with your draft and ask for critique
...
- **`pre-tool-use.sh`** (before each call) — kill switch, spending limits, prohibited-action + germline/constitution write-protection, and an early captain-law write diagnostic. On Mac, the security boundary is the OS sandbox plus the fixed-policy `captain-law-broker.py`; hooks are not treated as the boundary (`memory/skills/` remains read-only for officers).
...
### Schedules
**Clock-driven cadences are OWNED by `cabinet/services.yml`** (the fleet manifest) — read it rather than trusting a list here, which would rot every time the fleet changes. Examples from the manifest: `frontdoor-briefing` 07:30 + 19:30, `retro-trigger` hourly due-check, `officer-supervisor-mac` 2h loop re-arm. Event-driven work stays event-driven:
...
Only the MCP servers granted in `cabinet/mcp-scope.yml` are Cabinet tools. Do NOT use any other MCP servers that may be available on the Captain's profile — those are personal tools, not Cabinet tools. The core set:

- **Notion** — Business brain (strategy, brand, research, decisions)
- **Neon** — Product database (schema, queries, migrations); lane-CEO grants bind in `cabinet/mcp-scope.yml`
...
- **Cabinet** — inter-Cabinet comms (identify, presence, availability, send_message, request_handoff); consent-gated via `instance/config/peers.yml`. Granted in `cabinet/mcp-scope.yml` only when the server is actually wired on the deployment.
- **Vercel** — Hosting and deployment (preview, production)
- **Brain** — the captain's personal-sensing bridge (Flavor-A only; bound via `instance/config/sources.yml` → declared in `instance/config/extensions.yml`, scoped per officer in `cabinet/mcp-scope.yml`). Usage rules are MANDATORY when bound: `.claude/rules/brain-bridge.md` (the vault is read-first captain-truth; `queue_draft` is the only outbound path). Deployments with no personal source bound get fail-closed empties.
- **Comms** (`cabinet-comms`) — the officers' channel-agnostic, LLM-native surface TO the Captain: `send_card` / `edit_card` / `react` / `poll` / `set_status` / `pin` / `open_thread` / `stream_thinking` / `send_rich_card` / `read_feed`. Every tool routes through the Attention Gateway (charter class → dedup into a standing card → quiet-hours → external-comms floor → Chair T2) and the bound `ChannelAdapter`, then the feed journals it — so it is the ONE door and cannot bypass the charter. **Prefer `send_card` over raw channel sends** for anything situation-shaped (it dedups + edits-in-place instead of duplicating); use `stream_thinking` while composing, `react`/`poll` for lightweight signals, and `read_feed` (pass your role as `consumer` for an auto-managed cursor) to see what the Captain has already been shown. Lives in `framework/comms/` (foundation, channel-agnostic — Telegram is one adapter); scoped in `cabinet/mcp-scope.yml`; its design spec (the 2026-07-09 comms-MCP spec) is an instance planning document, archived out of the egg at export — see `docs/plans/ARCHIVED-NOTE.md`.
...
The **concrete model IDs and rollback commands** live in `instance/config/platform.yml` → "Model routing" — not duplicated here (the model is set from `CABINET_MODEL` / the start-officer scripts / `DEFAULT_MODEL` / each agent's `model:` frontmatter, never parsed from CLAUDE.md). One load-bearing gotcha: single-quote a model id wherever it reaches a shell (a `[1m]` suffix globs otherwise).
