<!-- CLAUDE-egg.md — ships AS CLAUDE.md in the public egg (claude-egg-swap transform in egg-export.sh). The operating context is fully captain-agnostic, so everything below this comment is BYTE-IDENTICAL to the live CLAUDE.md: any CLAUDE.md change lands here in the same commit. Enforced by cabinet/scripts/tests/test_claude_egg_lockstep.py — the twins had silently diverged on the candor bullet before that guard existed. Zero captain names, zero absolute home paths, zero real product values. -->
