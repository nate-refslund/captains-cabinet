<!-- CLAUDE-egg.md — ships AS CLAUDE.md in the public egg (claude-egg-swap transform in egg-export.sh). Since the 2026-07-16 rewrite the operating context is fully captain-agnostic, so this twin is byte-identical to the live CLAUDE.md below this comment — keep them in lockstep: any CLAUDE.md change lands here in the same commit. Zero captain names, zero absolute paths, zero real product values. -->
