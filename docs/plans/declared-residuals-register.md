...
appear in the tree without a row. Neither half is advisory. Rows cite a path
plus an ANCHOR TEXT and the test derives the line — see "Cites are
ANCHOR-resolved" below.
...
## Cites are ANCHOR-resolved, never line-pinned (2026-07-28)

`**Declared at:**` carries a repo-relative PATH with **no line number**;
`**Anchor:**` carries one backticked substring **per path, in the same order**.
The pin test searches the file for the anchor and derives the line itself. It
is RED when the anchor matches zero lines (the declaration was deleted,
reworded or moved out) and equally RED when it matches more than one — a cite
that resolves to two places is not a cite. Inserting lines above a declaration
is a no-op.

A `path:line` cite is now **refused at parse time**, so the old format cannot
come back by a new row copying an old one.

**Why.** `RES-007` cites a digest-bound FROZEN artifact that cannot be edited
in place, so every re-bind ceremony appends a note ABOVE its findings table and
the cited row moves down without changing a byte. The re-binds are structural,
not incidental: the `framework_production_noncomment_lines` budget is
zero-headroom, so any framework production line needs a row in
`cabinet/config/cognitive-architecture-contract.yml`, which sits in COG-4's
`restore_from_baseline` scope — so **every framework landing re-binds that
digest**. `RES-007`'s cite was hand-re-pointed twelve times, three of them
inside one day, each one a red CI job plus a diagnosis plus a commit; the row's
own note predicted every occurrence and proposed this exact fix twice before it
was done. The two legacy-exemption entries (`LEGACY_EXEMPT_ANCHORS`, line-keyed
as `LEGACY_EXEMPT` until this change) paid the same tax six times over,
from unrelated manifest rows landing above them, and are anchor-keyed now too.

**What this did NOT change.** The re-bind ceremony itself is untouched and
still required — a framework landing still re-binds the COG-4 digest. What is
gone is the *consequence* here: the ceremony no longer moves anything this
register has to chase.

**The anchor is now load-bearing, so choose it like one.** It must be unique in
its file, and long enough to stay unique — the ambiguity arm is what stops a
short anchor from silently resolving to the wrong occurrence. `RES-008` needed
lengthening on the day this landed (`officer-plist instance-leakage cleanup`
matched both the title and a quoted restatement 164 lines lower).

**A bare `RESIDUAL (RES-nnn)` tag is NOT an acceptable anchor**, and the
landing review proved why with a mutation rather than an argument. Delete
`RES-020`'s declaration from `memory/golden-evals/framework/fw-a14-stop-guard.sh`
and leave behind a pointer comment reading `# see RESIDUAL (RES-020) in the
declared-residuals register` — with a tag-only anchor the row resolves to the
POINTER and stays green while the declaration it names is gone, which is the
one thing this register exists to prevent. That file is outside the sweep
roots, so anchor resolution is its only sensor. Four anchors were tag-only
(`RES-016`, `RES-019`, `RES-020`, `RES-021`) and all four were extended to
carry declaration-specific prose; with the longer anchor the same mutation goes
RED. **The rule: an anchor must be text only the declaration itself would
write, never text a reference to it would write.** `RES-007`'s anchor was
lengthened to `| P1 | NOTE | Shadow-log replay window` for the same reason and
one worse — its artifact is frozen, so if a future re-bind note ever mentioned
the phrase, the cite would go ambiguous with no way to edit the file to fix it.

**Two sensor defects the sweep for this change turned up, both fixed here.**
Neither was about line numbers; both were found by asking what each assertion
does at its degenerate end.

- *Supporting cites were checked only for EXISTENCE.* A row's second and later
  cites were asserted to be "a line that exists", which any line number inside
  a long enough file satisfies — the anchor was never compared against them.
  Four rows carried such a cite. Every cite has its own anchor now.
- *Retiring a row could not be caught outside the sweep surface.* The retired
  check compared the row's coordinates against the sweep's discovered marker
  sites, and the sweep does not read `docs/`, `memory/` or
  `shared/interfaces/reviews/` — so retiring any of the SIX rows that cite
  those places while the declaration was still live would have passed silently
  (`RES-007`, `RES-008`..`RES-011`, `RES-020`). It now asserts the anchor is
  absent from the cited file, which is the property the check is named for.

## How to add a row

1. Declare the residual where the code lives, using the word token —
   `# RESIDUAL: <what stays open>`.
2. Add a `### RES-nnn` block below with every field filled: a bare path in
   `**Declared at:**`, and one anchor in `**Anchor:**` for each path cited.
...
test refuses a `retired` row whose anchor still appears anywhere in the file it
cites.
...
- **Declared at:** `cabinet/scripts/hooks/pre-tool-use.sh`
...
- **Declared at:** `cabinet/scripts/start-officer-mac.sh`
...
- **Declared at:** `cabinet/scripts/hatch.sh`
...
- **Declared at:** `framework/defaults/egress.yml`
...
- **Declared at:** `framework/frontdoor/action_undo.py`
...
- **Declared at:** `framework/probes/verifier.py`
...
- **Declared at:** `shared/interfaces/reviews/cognitive-core-phase-4-review.md`
- **Anchor:** `| P1 | NOTE | Shadow-log replay window`
...
- **Declared at:** `docs/plans/cog4-w1-u3-officer-plist-cleanup-PARKED-2026-07-23.md`
- **Anchor:** `u3 — officer-plist instance-leakage cleanup — PARKED (dated marker)`
...
- **Declared at:** `docs/plans/cog4-w3-u3-cortex-serve-adoption-park-2026-07-24.md`
...
- **Declared at:** `docs/plans/cog4-w3-u4-objectives-kernel-adoption-PARKED-2026-07-24.md`
...
- **Declared at:** `docs/plans/cog4-w4-u1-organ-schema-validation-PARKED-2026-07-24.md`
...
- **Declared at:** `cabinet/scripts/tests/lib_cog5_boundary_fixtures.py`, `cabinet/scripts/tests/test_cog5_arena_escape.py`
- **Anchor:** `DECLARED RESIDUAL (honest, W3-owned): a HOME override confines`, `DECLARED RESIDUAL (W3-owned): the override confines expanduser-based`
...
- **Declared at:** `cabinet/scripts/tests/lib_cog5_scoring_fixtures.py`, `cabinet/scripts/tests/test_cog5_sim_scoring.py`
- **Anchor:** `ratifies no such clause here. DECLARED RESIDUAL`, `self-consistent fabrication as a DECLARED RESIDUAL — pinned by`
...
- **Declared at:** `cabinet/scripts/tests/lib_cog5_scoring_fixtures.py`
...
- **Declared at:** `cabinet/scripts/tests/test_boundary_dynamic_forms.py`, `cabinet/scripts/tests/test_boundary_dynamic_forms.py`
- **Anchor:** `RESIDUAL HONESTY. The engine docstring documents what remains undetectable`, `DOCUMENTED RESIDUAL — what must still NOT be reported`
...
- **Declared at:** `cabinet/scripts/kill-switch.sh`
- **Anchor:** `DECLARED RESIDUAL (RES-016) — activate does NOT arm the marker yet`
...
- **Declared at:** `cabinet/config/state-persistence-policy.yml`
...
- **Declared at:** `framework/authority/classifier.py`
...
- **Declared at:** `framework/authority/policy_engine.py`
- **Anchor:** `KNOWN RESIDUAL (RES-019): this one still backtracks superlinearly`
...
- **Declared at:** `memory/golden-evals/framework/fw-a14-stop-guard.sh`
- **Anchor:** `RESIDUAL (RES-020): this guard tests a DEAD TWIN`
...
- **Declared at:** `cabinet/scripts/hooks/pre-tool-use.sh`
- **Anchor:** `RESIDUAL (RES-021): a shim that RECOGNISES the probe still defeats this`
...
### RES-022 — the one-door ratchet polices python and shell only, so two live TypeScript posters are outside its law

- **Phase:** attention-gateway P3 one-door ratchet — adversarial review of the fixture-shape sweep's degenerate-end arms (2026-07-29)
- **Status:** open
- **Closed:** the scan can no longer be silently EMPTIED or SHRUNK out from under the two `offenders == []` arms. `framework/tests/test_single_telegram_door.py` now fails when a scanned tree stops existing, when the walk returns nothing, when the door itself leaves the hits, when an `ALLOWED`/`KILLED` path names a file that does not exist, and — added by this review — when any listed path stops being REACHED by the walk. Measured both directions with caches purged: master's bytes stayed 2/2 GREEN with every telegram-touching file deleted; the two natural glob narrowings (`cabinet/scripts/**/*.sh` → `cabinet/scripts/*.sh`, and `framework/**/*.py` → `framework/frontdoor/**/*.py`, the latter taking the whole P3 kill-list off the surface) passed all four earlier arms and are RED now.
- **Open:** `SCAN_GLOBS` reaches `framework/**/*.py`, `cabinet/scripts/**/*.py`, `cabinet/scripts/**/*.sh` and `cabinet/admin-bot/**/*.py` — python and shell, in three trees. Two tracked files carry the raw `api.telegram.org` host outside that surface and appear on no list: `cabinet/dashboard/src/lib/docker.ts` and `cabinet/dashboard/src/app/api/telegram/provisioning-webhook/route.ts`. The module's opening sentence — "a new raw poster anywhere else is CI-red" — is therefore scoped to the globs, not to the repository. The new reachability arm is likewise anchored to the two lists, so a narrowing that drops only trees no listed path lives in stays green.
- **Why open:** widening the scan to `.ts` is not a test-only change. Both files are dashboard surfaces with their own provisioning contract, so bringing them under the ratchet means either routing them through the gated channel or ratifying two new SHRINK-ONLY allowlist entries with a stated reason — an allowlist decision, and the allowlist is spec §13-amendable, not review-amendable. Landing the widening inside a review of an unrelated fix would put an unratified allowlist entry into a germline-adjacent law.
- **Declared at:** `framework/tests/test_single_telegram_door.py`
...
- **Open:** the SURFACE half landed 2026-08-02: `answer_salience` is in the bridge's action set with `choice`/`name`/`same_as` typed, the Dashboard renders the candidates as a picker with the escape hatch and the merge, Telegram carries `/onboard salience <candidate>` and `/onboard salience other <name>`, and `propose_window` on BOTH surfaces can now carry `salience_relation` (a trailing `| same_thing` / `| elsewhere` on the Telegram folder command, two buttons on the Dashboard's off-target refusal). What remains is only the declaration itself: the paragraph in `framework/onboarding/journey.py` still says no shipped surface can send the answer, which is no longer true, and that file is schg-locked germline inside the COG-4 contract scope so this landing could not delete it.
- **Why open:** the row may not be retired while its anchor is still in the tree — `test_retired_rows_have_no_live_declaration` and `test_every_discovered_marker_is_registered` both hold it open, correctly: a stale declaration is worse than an honest one, and a reader of `journey.py` today is told something false. Deleting the paragraph is a germline content edit (landed-then-ceremonied, one Captain unlock/relock) and possibly a COG-4 re-bind, which is a unit of its own rather than a rider on a Dashboard change.
- **Declared at:** `framework/onboarding/journey.py`
- **Anchor:** `RESIDUAL, stated rather than implied: no shipped surface can send`
- **Retirement:** the surface conditions are MET (see Open). The one remaining act is deleting the now-false RESIDUAL paragraph from `framework/onboarding/journey.py`; in the SAME commit this row flips to `retired`. Do not retire it before then — the anchor resolving in the tree is what the two pin tests read.
...
- **Status:** retired
- **Open:** nothing. The split is the Unicode database — a Letter or a Number carries a word, a Mark belongs to the character before it, everything else ends the word — applied after a case fold and an NFKC normalisation, in ONE splitter that `name_tokens` and `tokenize` both read. Driven end to end on the fixed tree: answer `日本` ⇒ folder `日本` bound (`relation=matched`, `evidence=['日本']`); the same both ways for Cyrillic, Greek, Arabic, Hebrew, Devanagari and an accent the filesystem decomposed; `日本` against `韓国` still refused.
- **Why open:** it is not. The one thing that could have replaced a false refusal with a false BIND is a decorated name: a pictogram carries an invisible modifier, and counting a modifier as a word in its own right would make every decorated row share it. A Mark that starts a word is dropped, and an arm drives it (`test_a_decoration_is_not_a_word_two_names_can_share`).
...
### RES-027 — the refusal-detail lane is built and inert: the core's CLI never emits what it carries

- **Phase:** the journey is drivable end-to-end (2026-08-02)
- **Closed:** the receiving half exists and is exercised. `OnboardingBridgeError` carries a `detail`, `refusalDetail` reduces whatever the core printed to a THREE-FIELD allowlist (`target`, `window`, `relations`) with per-string and per-list bounds and no spread, and `/api/onboarding` returns it on a refusal — omitting the key entirely when it is empty, so absent never reads as present-but-blank. Driven both ways against a stubbed core: a detail carrying extra keys arrives with only the three, and a refusal with no detail arrives with `{}`.
- **Status:** open
- **Open:** `framework/onboarding/journey.py::_cli` prints `{"ok", "code", "error"}` and drops `JourneyError.detail`, so in production nothing ever reaches `refusalDetail` and the whole lane is inert. Measured 2026-08-02 by driving the real core: an off-target `propose_window` returns exactly those three keys. The operator-visible behaviour does not depend on it — the Dashboard builds its relation fix-up from `state.salience.target`, the folder it just submitted, and the mirrored `WINDOW_RELATIONS` — but a reader of the bridge could reasonably believe the core's own words are what the card renders, and today they are not.
- **Why open:** teaching the CLI to emit `detail` is a one-line change inside a file that is schg-locked germline AND in the COG-4 contract scope, so it is landed-then-ceremonied with a Captain unlock/relock and a possible contract re-bind — a unit of its own. Building the receiving half anyway was the smaller of two evils: the alternative was a surface that could only ever reconstruct, so the day the core does emit, its own words would still lose to a guess.
- **Declared at:** `cabinet/dashboard/src/lib/onboarding/bridge.ts`
...
`path:line` it is missing (the *discovered* site's coordinates — the sweep half
still reports line numbers, because it derives them from the tree rather than
carrying them).
...
They are exempted in the pin test (`LEGACY_EXEMPT_ANCHORS` in
`cabinet/scripts/tests/test_declared_residuals_register.py`, with a hard
maximum) so the list can shrink and never grow. **They are keyed by anchor
text, not by line** — as of 2026-07-28, for the same reason the rows are: they
had been hand-re-anchored six times each (233→235→242→248→258→267→283 and
653→…→722) by unrelated manifest rows landing above them, and the marker text
never changed once. Each anchor is the FULL comment line rather than the bare
`RESIDUAL SCRUB` token, because that token appears at both sites and the test
refuses an ambiguous anchor. Rewording the markers out of the sweep is blocked
for a good reason: that file is inside the frozen COG-4 review digest scope,
and changing marker TEXT would force a re-bind ceremony (adding unrelated
manifest rows does not).
