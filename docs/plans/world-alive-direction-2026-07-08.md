...
- **SUPERSEDED for the ISO world, 2026-07-30 (Captain's cycle ruling).** The ambient tint layer above is an OVERLAY, and an overlay decides per screen position — THE AMBIENCE STRUCTURE LAW replaced it with a colour remap (`cabinet/dashboard/src/lib/world/ambience.ts`). The bucket *ranges* and the window sky cuts stand unchanged; the DEPTH and HUE of the cycle no longer come from the sky table. Shown a strip that measured dawn at 88% and dusk at 82% of noon, the Captain ruled the cycle stylized beyond the art's own numbers: "deepen them so the day visibly turns — warm low light at dawn, amber at dusk, the way the cozy pixel games we're aiming at do it. The cycle should read at a glance." Shipped: 1 / 1.5 / 2 ramps of the art's own shade at dawn / dusk / night, and a split tone (cool shadow, warm light) at dawn and dusk. Night is unchanged. The derivation, every bound and the measurements are in ambience.ts and ambience.test.ts.
