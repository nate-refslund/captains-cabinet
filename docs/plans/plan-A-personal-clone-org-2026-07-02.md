...
- **TTL policy (A4.5):** `identity/constraint: none` · `commitment: closed+90d` · `deploy_fact: superseded-by-next` · `incident: 14d` · `unvalidated_reflection: 10d unless confirmed`. Expiry = set `valid_until`, never DELETE. Consumers: commitment-ledger nudge gate (kills the triple-nudge class), decision dossier (A5.2), the fidelity fence (as-of-T context).
...
The truthfulness gate (1b) maps every extracted claim to a slot or NONE; NONE + load-bearing ⇒ HOLD-AND-ASK. Verification battery: the two-colleague demo thread + 50-draft regression set, asserted before Stage 2 opens.
