...
- `canonical_refs(evidence) -> frozenset[str]` — mechanically extracts stable ids from evidence strings: vault-relative `.md` paths (strip ` — annotation` suffixes, `ref=` prefixes, quotes/whitespace), `cmt-<hex>` ids, `monday:<digits>`, calendar event UUIDs, correlation ids (`cabinet-proposal-id:<hex>`), normalized URLs. Deterministic, idempotent. Unit-tested against the *real observed corpus* — the five spellings of the testament evidence must all collapse to `{6-Commitments/owed_to_nate/cmt-fca6836e2844.md, cmt-fca6836e2844}`.
...
|---|---|---|
| **P1 Identity** ✅ built 2026-07-08 | situation.py; canonical refs wired into `propose_actions` dedup at compare time, both sides (`lane_dedup`/ledger-row changes proved unnecessary — `covered_evidence` already carries every open+decided card's raw refs) | PROVEN by pinned tests: the 5 testament evidence spellings collapse to one situation; 23 observed cards → ≤8 identity groups (union-find) and 10 first-presentations under live suppression semantics (suppressed cards donate no refs — transitive merge arrives with P4 standing cards); testament collapses to EXACTLY ONE card; full framework suite 3780 passed |
| **P2 World-grounding** | pre-propose probes; acted-overlay joined into gather from undo journal/ledger; resolution consequences | Acted testament situation → re-gather proposes nothing; probe-outage → `unknown` (card still proposes, never acts) |
| **P3 One door + feed** | bypass re-route → intake; `attention-submit.sh`; channel edit/upsert; feed journal; CI tripwire | Ratchet green (`api.telegram.org` only in channel.py + allowlist); every send/edit has a feed row; killswitch blocks all producers |
| **P4 Standing cards + charter** | charter schema/loader/default; gate charter-resolve; composer integration; terse card templates; banner-presentation move (§5) | Golden-render corpus; one situation = one message across 3 state flips; batch card at 02:00 lands in the 07:30 briefing, not the feed |
| **P5 Chair T2 + fallbacks** | judgment-request plumbing, dossier assembly, SLA timers, `(chair-offline)` marker | Simulated chair-down: floor class sends mechanically ≤ SLA, batch holds; T2 verdicts journaled |
...
## 13. Out of scope (follow-ups filed separately)
