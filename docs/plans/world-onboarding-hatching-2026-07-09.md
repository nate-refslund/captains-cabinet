...
| 12 | Flavor (`autonomy.flavor`) | "Does this box sense a person's estate, or only the org's own?" | **The memory chest**: org → a small book chest appears by the desk (OrgSource binding = the cabinet's own memory estate); personal → a shelf of the operator's own notes (LocalNotesSource over a declared folder, read-only). Both bind real recall since 2026-07-27; personal used to render the chest empty, which was the honest picture of a preset that shipped inert | `sources.yml` emitted on BOTH flavors, adapter per the generator's emission rule |
