...
deployment: mini-polads        # MUST equal CABINET_ID env (default "main"); mismatch ⇒ treated absent ⇒ guardian
...
lanes: {polads: sovereign}     # OPTIONAL per-lane override
...
**D7 Grants change ONLY via `sudo grant-apply.sh` in an unlock window (RUNTIME R1 wins).** Binder `grant NEED-x` ⇒ `approved_pending_apply` + one pasted sudo command. **KILLED: the Telegram silence-seal root grant-writer** (INTELLIGENCE §3.4/INT-8) — it turns Nate's Telegram session into a network-reachable authority-minting credential and breaches Decision B / its own Ring-0. `grant-apply.sh`: root re-exec guard → load NEED → refuse unless `approved_pending_apply` → refuse scope-mismatch vs recorded need scope → refuse ceiling class without `ceiling_ack` → `germline-lock.sh unlock` → append → validate via `grants.py --validate` (abort+restore on fail) → commit → lock → `mark(granted)` → receipt. Optional `unlock --only <file>` so grant windows don't de-posture the whole estate (BOUNDLESS medium). Zero-paste root grant-writer stays a separately-ratified v2 question.
...
| external_comms on Nate's personal surfaces | **KEEP** (ACT-NOT-DRAFT; grants loader structurally refuses flavor=personal external_comms) |
...
## §8 — NATE HANDOFF (ordered, Captain-only)
...
5. **First flavor-A sovereign lanes** the first needs-digest should argue for (guess: polads pm/calendar + internal_comms) — pure preference.
