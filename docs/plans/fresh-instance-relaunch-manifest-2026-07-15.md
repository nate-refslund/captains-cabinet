...
| `instance/roles/active/{comms-officer,cos,polads-ceo,stephie-ceo}.yml` | "Durable role state... encode lane_slug + auth level chosen at activation time" (`.gitignore`'s own words) — regenerated by `bootstrap-roles.sh` at first deployment, not hand-carried. See §3e on cos.yml/comms-officer.yml specifically. |
