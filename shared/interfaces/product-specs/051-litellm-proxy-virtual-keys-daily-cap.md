...
**Version:** v7.3 (AC #14 — M-DPO-2: litellm `log_requests` OFF, prompt-PII minimization) — v7.2 superseded

**v7.3 changelog — M-DPO-2 request-logging minimization AC added (FW-121 DPO-substitute pass, 2026-05-26):** the merged `proxy/config.yaml` ships `general_settings.log_requests: true` under a comment claiming it feeds the audit logger — a false premise: the FW-096 audit callback (`litellm_settings.callbacks`) receives usage/cost metadata via the callback hook regardless of this flag. `log_requests: true` is LiteLLM's *separate* built-in request-logging that writes prompt/response content to stdout → the container's docker json-log, which lives OUTSIDE the FW-097 erasure-governed SSOT (`customer-erasure.sh` only pseudonymizes `proxy/logs/audit/*.jsonl`, never the docker log). It therefore leaks customer prompt PII into a store that minimization (Art 5(1)(c)) forbids and erasure (Art 17) cannot reach. New AC #14 mandates `log_requests: false` (zero functionality loss) + a harness assertion. HARD pre-go-live gate; the FW-121 deploy README flags M-DPO-2 but `config.yaml` is FW-096/CPO-scope, so the value is pinned here (not silently flipped in the deploy PR).

**v7.2 changelog — path-pin deploy-semantics corrected (FW-121 PR #108 review, 2026-05-25):** the FW-121 docker-compose exposed that the v7.1 "both MUST resolve the same `LITELLM_AUDIT_LOG_ROOT` env var" was imprecise. FW-096 interprets it as the proxy-audit DIR (writes `<root>/<slug>`); FW-097 as the PARENT (reads `<root>/proxy-audit/`, writes `<root>/audit/`). The deploy sets the producer value ONE LEVEL DEEPER (= consumer/proxy-audit), NOT equal — equal breaks the ingest. AC #6 corrected to the offset semantic; the FW-121 compose (litellm=/data/logs/proxy-audit, audit/ingest=/data/logs) is the authoritative reference. Producer write-dir == consumer read-dir (proxy-audit) is the real invariant, not literal env-value equality.
...
**v6 changelog — routing mechanism corrected (CTO FW-096 design review + claude-code-guide verification, 2026-05-24):** AC #4 + AC #11(a) specified Sonnet/Opus routing via a per-request `X-LLM-Preference` header injected by `llm-routing.sh` based on client-side tool-call-depth. **That is UNBUILDABLE** — claude-code-guide confirmed Claude Code's `ANTHROPIC_CUSTOM_HEADERS` is **static per-session** (set at startup, cannot vary per-request by runtime state). Corrected to **per-request MODEL-NAME routing**: `llm-routing.sh` selects `model: claude-opus-4-7` (escalate) vs `claude-sonnet-4-6` (default) per request; the proxy routes by model name (natively per-request). AC #11(b) uncertainty-retry is unaffected — it's PROXY-SIDE (LiteLLM extracts the uncertainty field + retries with Opus), no client-header dependency. Caught at design-review BEFORE FW-096 build (CTO's model-name instinct was right; verified the WHY).
...
- **CTO #7 proxy.refslund.ai DNS + TLS:** Cloudflare + origin VPS pattern (TLS terminates at Cloudflare → forwards to the origin VPS; a local Caddy/nginx reverse-proxy routes to the internal service ports). Deployed via the **FW-121** refslund.ai-backend deploy stack (Hetzner docker-compose: proxy + audit-server + ingest + Redis). NOTE: NOT FW-098 — FW-098 is the customer-Mac concierge install; FW-121 is the backend deploy (corrected 2026-05-25 CPO+CTO scoping).
...
4. **Provider routing AC** — every request defaults Sonnet 4.6 (Captain msg 2540 cost-optimization). Officer overrides route via the **per-request MODEL NAME** (officer / `llm-routing.sh` requests `model: claude-opus-4-7` to force Opus, or `claude-sonnet-4-6` for default/deny-escalation); the proxy routes by model name (natively per-request). **[Mechanism correction 2026-05-24, claude-code-guide-verified: Claude Code's `ANTHROPIC_CUSTOM_HEADERS` is STATIC per-session, NOT per-request — a per-request `X-LLM-Preference` header that varies by runtime state is UNBUILDABLE. Model name is the per-request routing key. A static per-session per-officer default could use `ANTHROPIC_CUSTOM_HEADERS`, but per-request routing = model-name.]** Anthropic provider outage (HTTP 503 OR rate-limit cascade ≥3 retries) triggers fallback to OpenAI GPT-X or Gemini per LiteLLM provider-mapping config (fallback DISABLED until FW-100 DPA enumerates all providers, per I4). Fallback logged as `provider: openai-fallback`.
...
6. **Audit-log emission AC** — every proxy request produces a JSONL line per schema above to `proxy/logs/proxy-audit/<cabinet-slug>.jsonl`. **Path-pin (FW-096 merged ce61fca; deploy semantics corrected per FW-121 PR #108):** FW-096 `audit_logger.py` interprets `LITELLM_AUDIT_LOG_ROOT` as the proxy-audit **dir** and writes `<root>/<slug>.jsonl` (default `<proxy>/logs/proxy-audit/`); the FW-097 sidecar interprets the SAME env name as the **parent** (reads `<root>/proxy-audit/`, writes the SSOT `<root>/audit/`). The deploy MUST therefore set the producer's `LITELLM_AUDIT_LOG_ROOT` ONE LEVEL DEEPER than the consumer's (producer = `<consumer-root>/proxy-audit`) so both resolve to the SAME proxy-audit dir — NOT the literally-same value (equal values break the ingest). The FW-121 docker-compose is the authoritative reference (litellm=`/data/logs/proxy-audit`, audit/ingest=`/data/logs`). This is the raw per-request PRODUCER stream, distinct from the FW-097 hash-chained SSOT at `proxy/logs/audit/<cabinet-slug>.jsonl` (Spec 052, the consumer's own file — different by design). Log retained per FW-100 GDPR baseline retention policy (default 90 days hot, 5y general / 10y tax-relevant cold per Spec 055 v7.3 H4). FW-097 substrate consumes this stream for the customer-facing audit-trail surface.
...
7. **Customer dashboard visibility AC** — FW-101 dashboard sources from TWO surfaces, by purpose:
   ...
   - **Attribution surfaces** (per-officer breakdown table, last-7-days trend chart, per-model split) read **proxy-audit JSONL aggregated by day** — the JSONL is the right source for breakdown/history where the enforcer exposes no per-officer split.
   - DKK shown as FX-converted display per AC #5; USD is primary. (Rationale: FW-096 PR #99 review — `audit_logger.py` `cap_pct_used` is a **per-request** ratio `cost_raw / cap_usd`, explicitly informational, NOT cumulative; the enforcement counter is the team-budget. FW-101 must read the enforcer, not the audit field nor a JSONL re-sum.)
...
9. **Cost computation + Stripe Token Billing integration AC** — daily proxy-audit JSONL totals fed to Stripe Token Billing meter daily at 00:10 UTC (per Spec 054 v2 CTO #4 — 00:10 not 00:05, avoids the Spec 052 hash-chain checkpoint collision). Stripe meter ID per cabinet stored in customer record. Monthly Stripe invoice generated from meter; included in 25k DKK base + 5k DKK/employee subscription. Token consumption above included quota billed as overage. Specifics in FW-099 Stripe integration spec.
...
11. **Sonnet+Opus-advisor routing AC** (resolves self-review B3 + CTO #5+#6) — Sonnet 4.6 is default model for officer requests. Opus 4.7 invocation triggers, in priority order:
    - **(a) Officer-config / depth-based MODEL SELECTION** — `cabinet/scripts/lib/llm-routing.sh` (driven by `.cabinet/agent-instructions.md → llm_routing` config + a client-side tool-call-depth counter) selects the **per-request model name**: at depth ≥3 OR per the agent-instructions threshold, the officer requests `model: claude-opus-4-7`; otherwise `claude-sonnet-4-6`. The proxy routes by model name. **[Corrected 2026-05-24, claude-code-guide-verified: NOT a per-request `X-LLM-Preference` header — `ANTHROPIC_CUSTOM_HEADERS` is static per-session, so a depth-varying header is unbuildable. Per-request escalation = model-name selection, which is natively per-request and proxy-routable.]**
    - **(b) System-prompt-wrap self-classified uncertainty** (Phase 1 workaround for absence of Anthropic-native uncertainty signal) — Sonnet's system prompt asks model to emit structured output `{"uncertainty": "low|medium|high"}` per response. **LiteLLM (PROXY-SIDE) extracts the field; if `"high"` AND no opt-out flag set, the PROXY retries the request with Opus 4.7** (proxy-side retry — no client-header dependency, buildable as-is). Phase 2 may swap to Anthropic-native signal IF Anthropic ships one publicly. **Per A13 (don't seek permission from gatekeepers before you have leverage): Cabinet does NOT initiate Anthropic partnership outreach for native-signal access — we ship under value-add carve-out interpretation + wait for organic leverage (5-10 paying customers + revenue + relationship) before any vendor outreach.**
...
14. **Request-logging PII-minimization AC (M-DPO-2 — GDPR hard pre-go-live gate)** — `proxy/config.yaml → general_settings.log_requests` MUST be `false`. LiteLLM's built-in `log_requests` writes full prompt/response content to stdout → the container's docker json-log, which is NOT the FW-097 erasure-governed SSOT: `customer-erasure.sh` only pseudonymizes `proxy/logs/audit/*.jsonl`, never the docker log — so any prompt PII captured there is un-minimizable (Art 5(1)(c)) and un-erasable (Art 17). The FW-096 audit callback (`litellm_settings.callbacks`) receives usage/cost metadata via the callback hook independently of this flag, so `false` costs zero functionality. The compose `logging:` size caps bound DISK use (Art 5(1)(e)) but NOT minimization or erasure-coverage. `cabinet/tests/test-litellm-proxy.sh` asserts `general_settings.log_requests` resolves to `false` (loud fail on regression). If a future need ever forces it ON, the docker-json-log path MUST first be added to the erasure runbook AND proven body-free for the pinned LiteLLM version.
...
- **FW-101 dependency:** customer dashboard reads the **LiteLLM team-budget current-spend** for headline cap-status (the enforcer counter) and **proxy-audit JSONL aggregates** for attribution breakdown/history — see AC #7 (USD primary + DKK display per AC #5 per-billing-cycle FX).
