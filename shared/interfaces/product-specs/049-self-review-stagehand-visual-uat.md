...
**Version:** v3.2 (AC #24 — Redis-service-down semantics: cap-Redis-down → INDETERMINATE; semaphore-Redis-down → fail-open+WARN) — v3.1 superseded

**v3.2 changelog — Redis-service-down semantics AC pinned (PR #117 R3 conformance fold, 2026-05-31):** the Phase 3 runner-core PR raised a real conformance question — what happens on Redis-service-DOWN (the cap-event-chain HSETs + the M2 semaphore both depend on Redis as a service)? Existing canonical was: AC #12 + ARCH-1 had a graceful-degradation precedent (dep-missing = WARN-not-FAIL); AC #13 covered lock contention → `INDETERMINATE-CONCURRENCY-STARVATION` but was silent on Redis-service-DOWN as distinct from lock-contention; the build brief's "fail-safe NEVER fail-open" was scoped to correctness-critical deps. Crew's shipped design (PR #117, f041c34) correctly differentiated by criticality: cap-Redis-down → INDETERMINATE (irreversible cost per ARCH-2); semaphore-Redis-down → fail-open+WARN (resource-protection only, not correctness, per Spec 034 §2b.4 one-task-per-officer-window invariant). NEW AC #24 codifies this so future implementers can re-derive without context. Closes the canonical gap the PR #117 conformance review surfaced.
...
8. **Per-task step+token ceiling AC (C1 — sources named, was unimplementable):** each cap names its writer + data source (FW-016 removed byte-count tracking, so there is no naive per-task token counter):
   ...
   - **`agentTokensTotal`** (a TOKEN count — cap default 10M tokens) ← per-task DELTA of the wrapper HSET `cabinet:cost:tokens:daily:<date>` **token-count fields `<role>_input` + `<role>_output`** (NOT `<role>_cost_micro` — that field is micro-USD COST, the wrong unit for a token cap; cost is tracked separately by `visualUatCost`). Snapshotted at `/pickup-task` and diffed at each check. Officer-scoped ≈ task-scoped under the **Spec 034 §2b.4 one-task-per-officer-window invariant** (pinned here as the enabling assumption); if an officer multitasks, this over-counts conservatively (fails safe toward the cap).
...
21. **Test harness AC** (CRO F20 + COO MF/C/M fold + AC #24 Redis-down) — `cabinet/tests/test-spec-049.sh` covers 1:1 AC #1-24 PLUS: cap-bump audit-trail (now jsonl-only, NO captain-decisions write — C2 negative assertion); atomic-commit override jsonl; **the JF joint-failure determinism scenario** (all 3 failures co-fire → terminal==FAIL + override-path-closed + checkpoint-discard-on-build-change + permit-released-during-waits + cumulative-cost-persists); build-hash binding (`gate4BuildHash`/`checkpointBuildHash`/`selfReviewPassedSha` — ship-pr BLOCK when HEAD advanced); C3 conventional-commit bypass corpus (multiline/`-F`/`-c`/`bash -c`/chain/subshell + `--no-verify` block); semaphore crash-safe release (M2); **AC #24 Redis-service-down both paths** (cap-Redis-down → INDETERMINATE/exit-99; semaphore-Redis-down → fail-open + WARN + run-proceeds, no slot-key write). **Negative-assertion discipline:** every guard-asserting test must FAIL when the guard is deleted — verified by source-modify-rerun-FAIL evidence (PR #117 R3 toggle-test table). **Vision-API regression-pin:** at least one non-dry-run mock-path that exercises the real `visionFallback()` fetch end-to-end (PR #117 R3 added section `k`, 7 assertions covering http200-pass/fail, http429, http500, body-error). **≥23 assertions total** (raised from ≥15). Each AC has a dedicated scenario.
...
24. **Redis-service-down semantics AC (v3.2, PR #117 conformance fold)** — when Redis (the cap-event-chain + semaphore backing store) is DOWN as a SERVICE (distinct from lock-contention — that is AC #13's `INDETERMINATE-CONCURRENCY-STARVATION`), the runner-core differentiates by CRITICALITY class of the dependent state:

   (a) **Cap-Redis-down → INDETERMINATE (exit 99 `EXIT_SETUP_ERROR`).** The cap-event-chain (`cabinet:cost:tokens:daily:<date>` HSET + the `CAP_HIT`/`CAP_APPROACH` events) is correctness-critical: proceeding without cap-knowledge can blow the per-cabinet daily cap, and per ARCH-2 cost is *irreversible spend* (cost-cap BLOCK > token-cap WARNING). On Redis-down the cap-check cannot complete → fail-safe to INDETERMINATE, **never silent-default to 0/unlimited** (R2-Opus F7/F10 finding: silent-defaults on missing-config/dep are systemic false-PASS).

   (b) **Semaphore-Redis-down → fail-open + loud WARN.** The cross-officer concurrency semaphore (AC #13 M2 SET NX EX `cabinet:visual-uat:slot:{1,2}`) is *resource-protection*, not correctness — Spec 034 §2b.4 one-task-per-officer-window pool invariant means each officer has at most one in-flight task under normal operation, so the semaphore bounds cross-officer parallelism, not correctness. On Redis-PING-fail: `_warn "Redis unavailable — skipping concurrency semaphore (fail-open; no starvation protection this run)"` and proceed. Worst case = transient resource-overuse (a few extra concurrent Stagehand runs), not a correctness failure.

   **Why two semantics, not one:** AC #12 + ARCH-1 establish the graceful-degradation-WARN-not-FAIL precedent for dependency-missing cases generally; AC #13 covers lock contention but is silent on Redis-service-DOWN as distinct; the brief's "fail-safe NEVER fail-open" applies to correctness-critical deps (cap, Stagehand-config, leaf-libs). Cap-vs-semaphore reflects two real criticality classes. PR #117 (f041c34) implements both: `runner.sh` 242-247 (semaphore PING fail-open + WARN) and `runner.sh` 143-160 + `runner.js` F7 model-pricing-fail-throws (cap path → exit 99).

   **Test:** AC #21 harness exercises both — cap-Redis-down → assert exit 99 INDETERMINATE; semaphore-Redis-down (mocked PING-fail) → assert run proceeds + WARN emitted to stderr + no slot-key write attempted.
