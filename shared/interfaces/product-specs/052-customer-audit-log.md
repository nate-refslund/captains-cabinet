...
1. **Proxy-audit stream** (Spec 051 AC #6) — every LLM API request via LiteLLM proxy. The FW-097 sidecar reads this stream from `proxy/logs/proxy-audit/<cabinet-slug>.jsonl` — the `LITELLM_AUDIT_LOG_ROOT` dir the merged FW-096 `audit_logger.py` writes (ce61fca). **Both sides MUST resolve the same `LITELLM_AUDIT_LOG_ROOT` env var or the ingest is empty.** The sidecar transforms each FW-096 per-request emit into a Spec 052 entry; FW-096-only fields `request_pct_of_cap` + `status` have no 052 schema slot → drop or carry as `metadata`.
