...
**Version:** v3.10 (AC #13 NEW — public-checkpoint opaque-id keying: minted random id, fail-closed map, no slug in the permanent public sink) — v3.9 superseded
**v3.10 changelog — public-checkpoint opaque-id keying pinned (WORM PR #114 review, 2026-05-26):** my review of the merged WORM PR-1 (checkpoint.py, 1467113) surfaced that the public checkpoint (served `refslund.ai/audit-checkpoints` + the public immutable `refslund-cabinet-checkpoints` Git mirror) was keyed by the human cabinet SLUG — which would publish refslund's enumerable customer list + per-cabinet activity volumes to a PERMANENT IMMUTABLE public sink (Art 5(1)(c) minimization + confidentiality; a sole-trader slug is personal data; immutable = unretractable). New AC #13 requires keying by an OPAQUE per-cabinet id: MINTED random at FW-098 install (`openssl rand -hex 16`, NOT HMAC-from-slug — the public key must be stable forever + rotation-independent, and a salt-leak would de-anonymize), stored in the FW-099 customer record, read by `checkpoint.py` via a slug→opaque-id map, FAIL-CLOSED (no mapping → skip, never publish a bare slug), surfaced to the customer via the FW-101 authenticated dashboard. CTO agreed the direction + recommended the minted-id/map mechanism; this AC pins it. PR-1 is merged + inert (no public exposure); CTO HOLDS PR-2 (the public route + git push) until this pin — built against the resolved key choice. Cross-spec: FW-098/099/101 + checkpoint.py + the 055 ROPA. The 2 optional review minors (double SSOT read; non-served tmp dir) fold into CTO's checkpoint.py opaque-id change.
**v3.9 changelog — #237 write-side chokepoint analysis CORRECTED + SHIPPED (CTO catch on PR #113, 2026-05-26):** my v3.8 claimed a single chokepoint at `ingest_slug`-entry covers the write side. CTO's #237 build + Opus rounds proved that INSUFFICIENT: the SSOT write path is built by `hashchain.append` from `entry[cabinet_id]` — the RAW FW-096 record field (set by `_transform_fw096_to_052` via `raw.get('cabinet_id')`, ingest.py:89), a SEPARATE taint source from the validated filename stem. A valid-stem file carrying a record whose internal `cabinet_id` is a traversal still escapes on the SSOT write — and that record-field source is NOT filesystem-constrained, so the "LOW severity" I assigned in v3.8 was understated for the SSOT write. The shipped fix (f9bf91c, harness 73/0, 2 Opus rounds) is THREE chokepoints sharing one `validator.is_valid_cabinet_id` (DRY per #234, imported by app.py + ingest.py + hashchain.py): (1) `ingest_slug`-entry validates the stem (covers `_cursor_path` + proxy-audit read); (2) ingest per-record binds the transformed `entry[cabinet_id]` to the validated slug before `chain_append`; (3) `hashchain.append` — the UNIVERSAL SSOT-write chokepoint at the path-DERIVATION point — validates `cabinet_id` before building `audit/<id>.jsonl`, covering EVERY caller (ingest/POST/erasure/future), with read-side symmetry on `_last_entry_hash`/`verify` (non-slug → genesis, never builds a traversal read path). AC #10 corrected accordingly. Lesson: a path-traversal guard belongs at the path-DERIVATION point, validating the ACTUAL value used to build the path — distinct logical "ids" (filename stem vs record field) can carry independent taint sources feeding different builds, so guarding one upstream value does not cover a sibling. Conformance VERIFIED against f9bf91c.
...
12. **Test harness AC** — `cabinet/tests/test-customer-audit-log.sh` covers: entry emission per schema; hash-chain integrity (mock-tamper test breaks chain); PII minimization validator rejects oversized entries; customer dashboard widget loads correctly; full-history export CSV+JSON pagination; Article 15 endpoint 30-day SLA tracking; append-only filesystem ACL enforcement; erasure preserves hash-chain post-pseudonymization; **`pseudonym_marker_hash` recompute DETECTS post-erasure content-tampering of a pseudonymized entry (the AC #8(ii) regression test — tamper an erased entry's blanked field, assert verify() flags it; this guards finding #2);** customer-only access scope enforced (Phase-1: assert the GET endpoint is backend-mediated / rejects a bad or missing key; Phase-2 per FW-120: assert a key valid for cabinet A is REJECTED on cabinet B); retention cold-archive transition; **client↔server allow-list parity (§17, #234 sync-guard, merged 7c9075e): assert the officer-subset of `validator.py` `_ALLOWED_METADATA_KEYS` == `audit-emit.sh` `_AUDIT_ALLOWED_KEYS` — a one-sided edit on either FAILS the harness (silent allow-list drift → loud failure; also catches an unreviewed server-only key), enforcing the AC #3 fail-closed-allow-list invariant across both the client producer and the server backstop.** **cabinet_id traversal-reject (#236, pre-go-live): assert BOTH the GET path param AND the POST body reject a non-slug `cabinet_id` (`../../etc`, absolute path, null byte) with a 4xx before any filesystem access.** **ingest write-side traversal-reject (#237, pre-go-live) — BOTH taint sources: (a) drop a non-slug-NAMED file in the proxy-audit dir, assert `ingest_all` skips it (no `audit/<bad>.jsonl`, no `.cursors/<bad>.cursor`); (b) a valid-stem file carrying a RECORD whose internal `cabinet_id` is a traversal → assert `hashchain.append` refuses it (no SSOT write outside the cabinet's own `audit/` file) — the universal-chokepoint regression. Read-side symmetry: `_last_entry_hash`/`verify` return genesis / refuse for a non-slug id.** ≥15 assertions total.

13. **Public-checkpoint opaque-id keying AC (WORM PR-2 — pre-go-live HARD GATE on the public push/route, #114 review)** — the off-box checkpoint (served `refslund.ai/audit-checkpoints` + the public immutable `refslund-cabinet-checkpoints` Git mirror) MUST be keyed by an **OPAQUE per-cabinet id**, NEVER the human cabinet slug. Publishing per-slug would expose refslund's enumerable customer list + each cabinet's activity volume (`entry_count`) + cadence to a PERMANENT, IMMUTABLE public sink — an Art 5(1)(c) data-minimization violation (a sole-trader slug is itself personal data) + a commercial-confidentiality leak that cannot be retracted once in public Git history. Mechanism: (1) **FW-098 install MINTS a random opaque id** (`openssl rand -hex 16`) and stores it in the FW-099 customer record (Library Compliance Space) — MINTED + stored, NOT HMAC-derived from the slug: the public key must be stable FOREVER + rotation-independent, and an HMAC ties every historical checkpoint to a never-rotatable salt (rotate once → all past checkpoints unverifiable) AND a salt-leak would let an attacker HMAC every candidate slug to de-anonymize the public history. (2) **`checkpoint.py` reads a slug→opaque-id map** (a small JSON the install writes into the audit-server data dir) and publishes the served snapshot + the Git ledger keyed by the opaque id; the internal SSOT files stay slug-named. (3) **FAIL-CLOSED:** a slug with NO mapping entry is SKIPPED — NEVER publish a bare slug — so even a route/push that went live before the map is populated leaks zero slugs. (4) **FW-101 surfaces the opaque id** to the customer via the authenticated dashboard so they can locate + verify their checkpoint. This preserves both properties the public sink exists for — off-box independence + integrity-match — while removing the enumeration/volume exposure. Cross-spec: FW-098 (mint) + FW-099 (store) + FW-101 (surface) + `checkpoint.py` indirection + the FW-121/PR-2 deploy wiring; the Spec 055 ROPA records this public-checkpoint processing with the opaque-id as the minimization measure.
...
- **Hash-checkpoint publication endpoint compromised** — public verifier endpoint at refslund.ai/audit-checkpoints could be tampered with at the CDN/server layer. Mitigation: checkpoint hashes mirrored to a Git repository (refslund-cabinet-checkpoints — public, immutable Git history, keyed by an OPAQUE per-cabinet id per AC #13 — never the slug) AND signed with Captain's PGP key (Phase 2 per CTO #7; Phase 1 ships UNSIGNED — integrity verifiable via the published hashes + Git's own immutability). Customer can verify via Git history independently of refslund.ai availability.
