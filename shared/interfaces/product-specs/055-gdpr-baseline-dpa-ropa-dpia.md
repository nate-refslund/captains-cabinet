...
**Open Questions further reduced 3→2 via CTO #7 absorption of Q5:** Q1 (counsel ID) + Q2 (DPO appointment) gate-blocking; Q3 (retention duration) post-counsel-pass. **[ALL RESOLVED: Q1 cabinet-multi-officer-review (msg 2583), Q2 COO-as-DPO (msg 2737), Q3 5y/10y statutory (msg 2742), H3 Anthropic-wrapper accepted (msg 2744). ENTIRE LEGAL TRACK CLOSED — FW-100 unblocked end-to-end.]**
...
18. **Anthropic wrapper-terms risk-acceptance + defense dossier AC (v4 H3 fold — RESOLVED-ACCEPTED, Captain msg 2744 "accept then" 2026-05-24)** — customer DPA + ToS explicitly acknowledge Cabinet uses LLM provider through value-add proxy interpretation; customer accepts shared dependency on provider terms. **DPA/ToS risk-register entry (ship as-is):** standard category risk; low legal exposure (product-on-API is permitted usage, not raw resale; FW-115 dossier ready); dependency mitigated (LiteLLM FW-096 cost-cap + reversible provider-fallback + Spec 051 AC #8 proxy-degraded pause/notify/refund contingency); Anthropic named as sub-processor (transparency). Phase-1 business-risk call via cabinet multi-officer review (not external counsel, msg 2583 Q1); revisit at 5+ customers. Documented contingency plan: Spec 051 AC #8 proxy-degraded state on Anthropic enforcement (customer notification + service-pause + refund — fallback DISABLED per Captain msg 2583 Q5). Defense-preparation dossier per FW-115: `shared/interfaces/legal/anthropic-value-add-architecture.md` (Library Compliance Space mirror per A11) — value-add architecture documentation, customer-base evidence, technical-implementation-not-pure-resale evidence. CRO quarterly Anthropic-ToS tracking folds into existing 4h research-sweep cadence; material ToS changes trigger COO + Captain review. A13 reframing: don't outreach pre-leverage, DO prepare defense.
...
6. **H3 Anthropic wrapper/reseller terms risk-acceptance** → **RESOLVED 2026-05-24 (Captain msg 2744 "accept then"): Option (a) — accept the risk + ship under the value-add interpretation.** [History: Anthropic's 2026 terms restrict "single subscription authenticate API access on behalf of third-party end users"; Cabinet's value-add carve-out is operational risk (enforcement → all customers lose service). CPO/CoS recommended (a) over (b) per-customer-BYOK (which is the Phase 2 self-serve evolution anyway).] Mitigations: documented contingency (Spec 051 AC #8 proxy-degraded state), CRO quarterly Anthropic-ToS sweep, FW-115 defense dossier, reversible LiteLLM fallback. No longer gate-blocking. Revisit at 5+ customers.
