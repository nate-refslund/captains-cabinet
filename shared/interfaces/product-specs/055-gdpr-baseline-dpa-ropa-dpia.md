...
**Version:** v7.1 (H1 RATIFIED — COO-as-DPO) — v6 superseded
...
- **M3 DPIA Article 6(1)(f) balancing test missing** (compounds H4). If 7y retention preserved post H4, DPIA MUST document balancing test per WP29 Guidelines on Article 6(1)(f): identify legitimate interest specifically; necessity test (less-intrusive alternatives); balancing test (interest vs data subject rights + reasonable expectations); opt-out mechanism. DPIA structure (line 158-163) extended.
...
- **Q3 Cold-storage retention → LOCKED 7 years:** Danish bookkeeping default (overrides CPO P1 self-review finding that Bogføringsloven §10 specifies 5y; Captain accepts 7y conservative buffer per founder discretion). 10y still flagged for tax-relevant records per Skatteforvaltningsloven §47.
...
|---|---|---|---|---|---|
| Telegram DMs (text) | refslund.ai audit log (server-side) | 90 days | 7 years (compliance/billing reconciliation) | Customer dashboard view | None (internal audit only) |
...
| Audit log entries (proxy + officer actions) | refslund.ai server-side | 90 days hot | 7 years cold | Customer dashboard view | None |
...
6. Cold-storage handling: 7-year compliance hold preserved (anonymized), legal basis = Article 6(1)(c) compliance with legal obligation (Danish tax/accounting law) + Article 17(3)(b) — retention required for legal claims defense
...
3. **DPIA AC** — DPIA record-of-record in **Library Compliance Space** (canonical per A11); covers Article 35(7) all mandatory elements (description of processing, necessity + proportionality assessment, risk to rights + freedoms, mitigation measures including A12 officer-in-loop-on-architecture process anchor per v5 P1 fold). Generic-product DPIA + per-customer addendum trigger documented. **Article 6(1)(f) balancing test documented per v4 M3 fold** for any retention beyond statutory minimum (pending Captain H4 ratification of (a) statutory reduction vs (b) keep 7y with balancing).
...
5. **Data-handling matrix AC** — public page at `refslund.ai/data-handling` (Next.js MDX render per v5 CTO #5; source at `cabinet/customer-templates/` per Spec 053 CTO #6 + Spec 055 v5 SSOT cleanup) carries the 8-row matrix above. Customer dashboard surfaces retention-status per data type (90-day audit log countdown visible per FW-101 / Spec 056). PII matrix updated for v6 H4 statutory-reduction outcome pending Captain ratification.
...
11. **Captain final ratification AC (post v4 H1+H3+H4+v6 H5 cycle)** — Captain reviews + ratifies: (a) sub-processor list (msg 2583 Q5 Anthropic-only Phase 1 RATIFIED), (b) DPO role assignment (RESOLVED msg 2737 2026-05-24: COO-as-DPO; supersedes the msg-2583 CoS-as-DPO answer after the v4 H1 Article 38(6) reopening), (c) retention durations (msg 2583 Q3 7y RATIFIED but v4 H4 reopened → pending 5y/10y statutory reduction per CPO recommendation), (d) Annex III ToS clause (msg 2583 Q4 reader-friendly tone RATIFIED), (e) Anthropic wrapper risk-acceptance (v4 H3 NEW pending Captain), (f) iCloud Drive removal from pre-install checklist (v4 H5 Option (a) clean removal, no Captain ratify required for removal — only Option (b) add-as-sub-processor needs Captain). Captain ratifications recorded in `shared/interfaces/captain-decisions.md`.
...
- **Anonymous data after erasure** — 7-year compliance hold retains anonymized billing records. Anonymization = irreversible (random-token substitution for customer ID; no re-identification path). Documented in erasure runbook step 6.
...
3. **Cold-storage retention duration** → **REOPENED per CRO H4 — 7y beyond Bogføringsloven §10's 5y statutory lacks Article 17(3)(b) basis; "founder conservative buffer" not recognized GDPR retention basis.** CPO recommends **(a) reduce to statutory: 5y general billing + 10y tax-relevant** (clean Article 17(3)(b) basis, no Article 6(1)(f) balancing required). Alternative: (b) DPIA Article 6(1)(f) balancing test; (c) Captain explicit founder-liability acceptance in captain-decisions.md. Captain ratifies before signup-live. **GATE-BLOCKING.**
