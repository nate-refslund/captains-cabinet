...
> Maintained by CPO. Last refinement: 2026-05-25 22:00 UTC (cron — substrate COMPLETE + FW-121 deploy stack BUILT + spec-conformance-APPROVED (PR #108); allow-list parity closed (#234 validator.py + #107 sync-guard, Spec 052 v3.6); FF-pull drift-backstop shipped (#192/#109). Remaining = 3 Captain decisions (below). No new research briefs since 05-23).
> **Active phase: Phase 1 commercial CODE SUBSTRATE COMPLETE — everything buildable is merged + reviewed (incl. the FW-121 Hetzner deploy stack, PR #108 spec-conformance-APPROVED). Remaining blockers are CAPTAIN decisions, not engineering: (1) private commercial repo (board #184) → FW-099 signup + FW-101 dashboard; (2) Hetzner Frankfurt VPS (task #191; architecture ratified) → the proxy + audit deploy (FW-121); (3) the COO-path (CoS-surfaced) — the passive over-trim makes 2 review gates un-meetable: the FW-121 security pass (Opus-substitutable) + the coo-dpo.md DPO-compliance read (NOT substitutable, Spec 055 H1); NON-critical-path, resolves alongside 1+2. + Mac Migration (Specs 057-065 ready; Captain-hands-on). Legal track CLOSED (Spec 055 v7.3). Spec 049 Ph5 (C3) done.**
...
**Customer-#1 go-live = 3 CAPTAIN decisions (engineering is DONE):** (1) **#184** private commercial repo → unblocks FW-099 signup + FW-101 dashboard; (2) **#191** Hetzner Frankfurt VPS provisioning + proxy.refslund.ai DNS/TLS → unblocks the proxy + audit-server deploy (FW-121 deploy stack BUILT + spec-conformance-APPROVED PR #108: docker-compose + Caddy-origin-behind-Cloudflare + non-root + per-file-append-only; architecture ratified Hetzner Frankfurt; held on the VPS + the COO security pass); (3) **COO-path** (CoS-surfaced) — the passive over-trim makes 2 review gates un-meetable: the FW-121 security pass (Opus-substitutable, atop CTO 2 Opus deploy-rounds + my conformance) + the coo-dpo.md DPO-compliance read (NOT substitutable, Spec 055 H1 DPO authority). NON-critical-path (resolves alongside 1+2). All 3 surfaced to Captain by CoS. CLOSED since last refinement: allow-list parity (#234 validator.py + #107 sync-guard, Spec 052 v3.6 AC#12), FF-pull drift-backstop (#192/#109). Remaining non-blocking follow-ups: cap-reaction design (CTO), #236 app.py cabinet_id traversal (pre-go-live).
