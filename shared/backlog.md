...
> Maintained by CPO. Last refinement: 2026-05-31 22:00 UTC (cron — Spec 049 multi-PR arc CONVERGED this session: PR #117 runner-core MERGED f041c34 + my v3.2 AC #24 spec d8dcaa3 + PR #118 cleanup MERGED d7427a2 + PR #561 Phase 2b MERGED c0d0fdfa on Sensed. CoS #198 install shipped 3464f59 (Stagehand v3 + gitleaks + ast-grep exposed). Readiness checklist refreshed + force-added 9c75a5d (gap-closing + L3 git-versioning). FW-077 PR #84 regression flagged to CoS via Neon-leak (not cause for rotation per A8 refinement #2). 5 sibling memory lessons banked, propagating as active disciplines.) **Build status: commercial security/WORM cluster COMPLETE + Spec 049 self-review arc COMPLETE** (pending deferred residual cleanup). COO-path RESOLVED via workaround. Remaining = **2 Captain founder-actions** (#184, #191; #191 carries Cloudflare DPA/EU-config + checkpoints-repo+token deploy-prep sub-items) + the decouple-#197 question CoS surfaced. CTO + cabinet productively shipping infra-unblocked work during the ~111h Captain-gated quiet. origin @ d7427a2.
...
> Research absorbed (prior cycle, still current): CRO competitive sweep (Devin $20 + Anthropic enforcement), audience psychology digest (trust-gap #1 + 85/15 framing), Anthropic ToS Q1 sweep (value-add carve-out strengthened), Mac-native pre-staging (TCC code-signing — now folded into Spec 058 1.8 body + committed e8af6e9), Claude Code daily catchup (Spec 049 v3.2 ast-grep + /code-review boundary, staged). **2026-05-27 autonomous-CU/browser deep brief (CRO) — TRIAGED:** bounded reversibility-gated autonomy framing ADOPTED (validates A1/A12 + the May defer); Spec 049 VALIDATED (bounded review-only Stagehand = the production-ready zone, no change); asks B (AXTree-MCP pilot) + C (reversibility-classification) parked as spec candidates pending Captain decisions (product input given to CoS for surfacing).
...
**Customer-#1 go-live = 2 CAPTAIN founder-actions (engineering is DONE):** (1) **#184** private commercial repo → unblocks FW-099 signup + FW-101 dashboard; (2) **#191** Hetzner Frankfurt VPS provisioning + proxy.refslund.ai DNS/TLS → unblocks the proxy + audit-server deploy (FW-121 deploy stack BUILT + spec-conformance-APPROVED PR #108: docker-compose + Caddy-origin-behind-Cloudflare + non-root + per-file-append-only; architecture ratified Hetzner Frankfurt). **#191 deploy-prep sub-items (ride with #191, NOT separate decisions; CoS folds into the #191 task description + 07:00-briefing pre-engagement view):** Cloudflare DPA + EU-localization config (Art 44); create the public `refslund-cabinet-checkpoints` repo + write-scoped token (WORM off-box anchor — served snapshot works without it). Both surfaced to Captain by CoS.
...
| **Off-box WORM** | emit logic + opaque-id keying + deploy-wiring (sidecar 00:05 cron + Caddy route + public git push) | CTO/FW-097 | ✅ **COMPLETE** — 3 PRs all CPO-reviewed-APPROVED + MERGED: #114 emit (1467113), #115 opaque-id keying (ba2648b, Spec 052 v3.10 AC#13), #116 deploy-wiring (f3ed6da: Caddy serves opaque-keyed checkpoints `:ro` ONLY, EXDEV loud-fail, token-safe redacted push, 39/0). FOUNDER-ACTION (rides with #191): create public `refslund-cabinet-checkpoints` repo + write-scoped token → `AUDIT_CHECKPOINT_REMOTE`. CPO follow-ups: pin the id-map contract in the FW-098 spec (on #184) + add the public-checkpoint row to the 055 ROPA |
...
### Spec 049 self-review arc — ✅ CLOSED (2026-05-31, multi-PR convergence)
Full arc converged this session: **PR #117** runner-core MERGED f041c34 (3-round Opus); **PR #118** cleanup MERGED d7427a2 (residual #5 _existingError clear-on-PASS + AC #24 section (l) harness + 5 other residuals); **PR #561** Phase 2b MERGED c0d0fdfa on STEP-Network/Sensed (3-round Opus). My **v3.2 AC #24** spec amendment landed d8dcaa3 (Redis-service-down semantics codified: cap-down → INDETERMINATE per ARCH-2; semaphore-down → fail-open+WARN per AC #12 + §2b.4). CoS shipped **#198** (3464f59) exposing Stagehand v3 + gitleaks + ast-grep to officers.

**Deferred follow-ups** (CTO post-merge cleanup task; B5 in my lane):
- **B5 — anti_patterns prose-vs-spec text-grep semantics** (SPEC AMENDMENT, CPO lane when CTO files).
- **m7** — atomic writer for selfReviewIterationCount.
- **m5** — audit_allow_high coded-enforcement.
- **P1a/P1b** (xhigh effort + /code-review-vs-Gate3 boundary): verify post-merge against PR #561 SKILL.md (may already be folded).

**5 sibling memory lessons banked this session:** verify-canonical-before-re-greenlight, negative-assertion-harness-pattern, adversary-sample-crew-claims, deploy-pr-serve-scope, shared-fetch-head-clobber. All visibly propagating as active disciplines (Crew citing them in code comments, CTO briefing with them, R1-Opus independently verifying them) — memory → discipline transition demonstrable.

**Flagged FW-item to CoS (post-Phase-2b-merge routing to CTO):** FW-077 PR #84 regression — Neon connection-string leak via bash-tool background-job notices (same class as memory_queue_embed). Not cause for rotation per A8 refinement #2; framework-hygiene investigation only.

### FW-115 H3 defense-dossier (CPO authors) — DRAFTED ✓
- **Dossier DRAFTED + maintained:** `shared/interfaces/legal/anthropic-value-add-architecture.md` (v3, 2026-05-23, 225 lines — 5-criterion mapping table w/ Anthropic's own published value-add definition + CRO Q1 ToS-sweep + competitive-sweep M1/L3 enforcement-context folds). CPO maintains; CRO quarterly Anthropic-ToS sweep flags material changes.
- **Remaining (USE-gated, NOT a draft task):** CRO+CoS adversary review + Library Compliance Space record — gated on 1+ paying customer OR Anthropic engagement (A13 don't-seek-permission-pre-leverage). Draft is ready to deploy the moment Anthropic inquires; no proactive send.
...
### Autonomous-CU/browser brief (2026-05-27, CRO) — incorporated
- **Spec 049 VALIDATED, no change:** Gate-4's bounded, review-only, preview-URL Stagehand use is exactly the production-ready zone the brief endorses (not the >15-step reliability cliff; not consequential-write autonomy; preview-auth not bot-detected — so the Cloudflare mid-flow-scoring + self-hosted-IP-block concern is out of scope, as the spec's Local-not-Browserbase pick already reflects). WebVoyager-now-mid-pack (~97-98% ceiling) = awareness only; the conservative 85.8% cite stands.
- **[parked — Captain ask B; CoS framed proceed-unless-object] AXTree-MCP Mac-native desktop primitive** — bounded pilot (≤5 steps, reversible, sandboxed, credential-free, measured on REAL multi-step success). ~10× faster than vision (macOS AXTree). CPO-spec when CoS routes it; sequences with Mac migration (057-065). File the FW# on greenlight (don't pre-fabricate).
- **[parked — Captain ask C] Reversibility-gated action classification** framework primitive (classify reversible/irreversible; explicit gate on irreversible). CoS+CTO primary; Spec 049 ARCH-2 (cost-cap BLOCK = irreversible-spend gate) is the shipped precedent. CPO specs any product-facing slice.
- **[confirmed — Captain ask D]** full unattended autonomy on consequential officer flows STAYS deferred (27-50% real multi-step reliability + injection/credential safety + A12). Spec 049's bounded use is the evidence.
