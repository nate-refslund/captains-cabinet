...
- **Status:** SHIPPED 2026-04-17 (commit `8898ef5`, same day as Captain filing 15:50 UTC → ship 16:14 UTC). All 4 fixes live in `pre-tool-use.sh`: per-cabinet override via `_cfg_get daily_per_officer_usd` (line 188 reads `instance/config/platform.yml`), stderr-on-block on all 3 BLOCKED exit-2 paths (lines 229/252/273 `>&2`), Telegram whitelist with 10/hr sub-cap (lines 214-237, `IS_TELEGRAM_COMMS` bypass), CoS 3× carve-out (`coordinating_officer_multiplier` lines 206-210 with explicit note in block message line 251). Golden eval coverage: **EVAL-003** pipes synthetic JSON (officer=cos, cost_micro=999999999, skip-when-cap=0) → asserts exit=2 + stderr contains `BLOCKED.*officer=cos` (primary FW-002 assertion); **EVAL-007** pins the stderr invariant structurally (every `exit 2` in pre-tool-use.sh must have `>&2` on preceding meaningful line — any future silent-exit regression fails the eval); **EVAL-015** pins FW-032 Telegram invocation anchor (guarantees `IS_TELEGRAM_COMMS=1` only fires on real Telegram tool calls, not on filenames inside CMD). Backlog drift: status read "Proposed" for ~1 week post-ship; caught during 2026-04-24 CTO Paused/Proposed backlog hygiene sweep.
...
- **Status:** PARTIAL-SHIPPED 2026-04-24 — sub-scope #2 (EVAL-022 Check 1 regression guard extension to EV17) shipped this commit. Sub-scope #1 (full EVAL-017 migration to direct-invocation pattern) remains DEFERRED pending eager maintenance-window work OR operational trigger (post-tool-use.sh infra-gate anchor gaining `'\''`). Defense-in-depth is now in place: any future edit that adds `'\''` to the post-hook AND leaves the EVAL-017 sed extractor surviving will fail EVAL-022 Check 1 with explicit error message naming EV17.
...
- **EVAL-022 coverage AFTER this PR:** Check 1 (line 1716-1730) now extends to EV17 — the `-B5` context grep matches `EV17_` prefix naturally (EV17_FAILURE assignments + EV17_INFRA_ANCHOR_COUNT all appear within 5 lines of the surviving sed extractor at line 1416). Error message updated: `EVAL-011/013/017 still uses the fragile sed extractor. ... migrate EVAL-011/013/017 to direct hook invocation (FW-046 pattern; see FW-052 for EV17 scope).` Check 3's `for ev_prefix in EV11 EV13 EV15 EV16` loop still excludes EV17 by design (Check 3 asserts the 4 FW-046-migrated evals stayed migrated; EV17 is not yet migrated and including it would cause immediate fail — correct until sub-scope #1 lands).
- **Remaining sub-scope #1 (DEFERRED):** Full migrate EVAL-017 extractor → direct hook invocation (FW-046 pattern). Replace line 1416 sed extraction + positive/negative matrix loops at lines 1422-1462 with a `ev17_infra_probe()` function that invokes `post-tool-use.sh` directly and asserts on the infra-gate stdout marker (`⚠️ INFRASTRUCTURE REVIEW GATE:` at post-tool-use.sh:502). Harness complication: the infra-gate emit has TWO conditions — (a) FW-035 CMD anchor match AND (b) `git diff --cached` has files matching the critical-file regex. A probe would need to either (i) stage a critical file in a throwaway index before each probe or (ii) accept that the probe tests the COMBINED gate behavior, not the anchor in isolation. Option (ii) is the simpler harness; anchor-in-isolation is already pinned structurally by EV17_INFRA_ANCHOR_COUNT and EV17_FW035_MARKER checks at lines 1398-1413.
- **Effort (sub-scope #1):** S-M (~45min-1h: ev17_infra_probe + stage-setup harness OR combined-gate probe + Check 3 extension to include EV17 + harness verification).
- **Owner:** CTO. Priority: LOW (current usage zero; Check 1 guard is now in place). Operational trigger OR maintenance-window eager migration both acceptable.
...
- **Status:** **SHIPPED 2026-04-27** — verified already-fixed by FW-042 v3.7.1's CMD_STRIPPED preprocessing (quoted spans stripped before keyword scan, so data-position keywords pass through). New regression-pin harness at `cabinet/tests/hook-regression/fw057-notify-officer-argv.sh` (30 probes: 22 ALLOW + 8 BLOCK) wired into `run-hook-regression.sh` aggregator (now 12/12 harnesses green). Originally Proposed 2026-04-25 with 3 live blocks reported; survey 2026-04-27 found behavior had been incidentally fixed by FW-042 v3.7.1 ship 2026-04-25 (likely overlapping window). No code change required; harness PINS the contract so future CMD_STRIPPED edits can't silently regress.
...
- **Status:** **SHIPPED 2026-04-28** — Option A (entrypoint extension) chosen. `cabinet/scripts/entrypoint.sh` chown block now also covers `instance/memory/`, `cabinet/scripts/`, `.git/`, `cabinet/.env`. Next first-rebuild scenario will leave the container fully self-sufficient on UID 60001 without requiring a host-side chgrp sweep. `secrets/` deliberately not chowned (mode 0640 cabinet-group routing per `memory/skills/evolved/cross-uid-host-container-routing.md` is the secure-by-design pattern; chown'ing would break that). Originally Proposed 2026-04-25.
...
- **Status:** **SHIPPED 2026-04-28** — root cause was `this_cabinet_capacity()` only reading `platform.yml capacity:` key + `CABINET_CAPACITY` env, both empty on Personal Cabinet → fell through to `default="work"`. Fix: lookup chain now reads `instance/config/active-preset` (deployment's source-of-truth for preset assembly) BETWEEN env override and platform.yml fallback. Personal Cabinet's `active-preset=personal` now flows through automatically. 5 unit tests added covering all 5 lookup paths (env override / active-preset wins / missing preset → platform.yml / nothing → 'work' / empty preset file falls through). 25/25 mcp-server tests pass.
...
- **Status:** ALL 3 PHASES SHIPPED 2026-04-26. Phase 1: indexer + scaffold-entry + 24-block retro-fit + pre-commit freshness gate. Phase 2: query.sh keyword retrieval + pre-captain-dm UserPromptSubmit hook + 60s identical-block dedup + capability flag. Phase 3: eval.sh + 5-anchor fixture set + Cabinet CI wire. AC #13 LLM-eval comparison deferred as Phase 3.5 follow-up — deterministic "rule retrieved" assertion shipped as floor. File artifacts + scripts only, zero schema reach. Reversible.
...
### FW-117 — FLAG: reinstate the A1–A5 master-directive anchors (+ I-W-001 / old P-* patterns) into captain-rules-index (P1, Captain decision)
- **Status:** FLAGGED — Captain's call. Surfaced 2026-06-25 during the principles-over-specifics behavioral collapse (`docs/principles-over-specifics-audit-2026-06-25.md` BUG 1). **Do NOT auto-reinstate** (audit + task directive).
- **What happened:** `shared/interfaces/captain-rules-index.yaml` was stale — it carried 24 old-gen entries (A1–A5 anchors, I-W-* intents, P-* patterns) but **zero** of the current session-encoded patterns, because the current `captain-patterns.md` entries never had `<!-- index: -->` blocks (the indexer only parses those blocks). The index has now been regenerated from the live patterns → **15 current patterns indexed**. As a side effect, the 5 always-injected master-directive **anchors (A1–A5)** + `I-W-001` + the old `P-Build-Our-Own / P-Captain-Posture / P-Personal-Work-Parity / P-Proactive-Task-Creation / P-Stay-Productive` patterns dropped out of the index, because their bodies exist **nowhere in the current source `.md` files** — only in the (now-overwritten) stale YAML and in the eval fixtures.
- **Why this matters:** anchors are the `section: anchor` entries `query.sh` injects on **every** Captain DM (always-returned, not scored). A1–A5 are Captain-ratified master directives with real correction lineage (reversibility-gated autonomy msg 1791/1935/1968; captain-posture; build-vs-buy; personal-parity; stay-productive). Until reinstated, officers get **zero anchors** injected at turn-start — a behavioral regression vs. before this task. Also: **`cabinet/scripts/captain-rules/eval.sh` is now 0/5** (was 5/5) — its 5 golden fixtures assert exactly these anchors/patterns. **The red eval is intentional, pending this decision — not silent drift.**
- **The decision:** these anchors look like the clean PRINCIPLE layer the session-specifics should sit beneath (the audit's own framing). They are NOT redundant with the 15 current patterns — they are higher-level. Options:
  1. **Reinstate as anchors** (recommended) — re-add A1–A5 (+ optionally I-W-001 and the old P-* patterns) to `captain-patterns.md` / `captain-intents.md` as proper entries WITH `<!-- index: -->` blocks (`section: anchor` for A1–A5), re-run the indexer, eval returns to 5/5. Possibly fold P-Build-Our-Own≈A3, P-Personal-Work-Parity≈A4, P-Stay-Productive≈A5, P-Captain-Posture (master) — dedupe anchor vs pattern overlap while reinstating.
  2. **Retire them** — accept they're superseded by the current 15 patterns + constitution-base; then DELETE the 5 eval fixtures + the README anchor references so eval goes green honestly (no orphan assertions).
  3. **Partial** — reinstate A1/A2 (register + reversibility, the most-corrected) only.
- **Recommendation:** Option 1 (reinstate A1–A5 as anchors; fold the duplicate P-* into them). They are the principle layer, they were always-injected, and they have the strongest correction lineage. This is a content/governance call, so it's left to the Captain rather than auto-applied.
- **Recoverable content (verbatim from the pre-task index, ready to paste back as `<!-- index: -->` blocks):**
  - **A1** `[anchor]` Reversibility-gated autonomy — triggers `["sign-off","approve","OK to proceed","awaiting","want me to wait","ready for your review","reversible","irreversible","should I","go-ahead","let me know if you want","pending your"]` — *Default = SHIP, not GATE. If reversible in <5 min, CoS owns the call. Only gate genuinely irreversible actions (payments, public announcements, production DELETE, contracts, relationship-shifting). Detect gate-language and rewrite as FYI.*
  - **A2** `[anchor]` Captain Posture (comms register) — triggers `["IDs","paths","timezone","tech-talk","tech talk","abbreviation","jargon","casual","common words","plain language"]` — *Casual conversational register. No IDs, paths, timezone abbreviations, or tech-talk. Send files as Telegram attachments, not path pointers. Status only on material change or escalation. Trust the principle, don't enumerate.*
  - **A3** `[anchor]` Build-our-own > add-dependency — triggers `["dependency","dependencies","SaaS","MCP","third-party","vendor","build vs buy","native","paid API","integration","use X for Y","should we use"]` — *Default native; only adopt deps when complexity is genuinely prohibitive. Every dependency is a strategic risk that compounds. Surface build-vs-buy explicitly when proposing tools/MCPs/SaaS.*
  - **A4** `[anchor]` Personal-Work parity — triggers `["all coaches","personal coach","personal cabinet","parity","shared infra","extend to personal","personal-assistant","PA","both cabinets","peer cabinet"]` — *Personal Cabinet officers get full parity by default on shared infra (research APIs, /tasks, MCPs, schemas). Default-include all 4 Personal coaches + PA. Skip only with a domain-specific reason.*
  - **A5** `[anchor]` Stay productive, never idle — triggers `["idle","productive","stay productive","waiting","ratification","next step","hold","pause","do that","go ahead","ship the next"]` — *After Captain gives direction, ship the next concrete deliverable — don't wait for re-confirmation. Status only on material change or escalation. Idle time = lost velocity.* (NOTE: overlaps the current `never-stop-during-an-active-task` pattern — reconcile if reinstated.)
  - **I-W-001** `[intent]` Plain-language first, tech second — triggers `["common words","plain language","no IDs","no paths","no tech","no technical","decoding","acronym","technical words","in common words"]` — *Open every Captain reply with plain-language summary. Tech detail comes after or in a separate artifact. Acronyms expanded on first use.* (goes in `captain-intents.md`.)
  - Old patterns **P-Build-Our-Own** (≈A3), **P-Captain-Posture** (master), **P-Personal-Work-Parity** (≈A4), **P-Proactive-Task-Creation**, **P-Stay-Productive** (≈A5) — full excerpts/triggers preserved in git history at `git show HEAD~1:shared/interfaces/captain-rules-index.yaml` (pre-collapse).
- **Owner:** Captain decides; CoS applies the chosen option (re-add blocks + re-run `index.sh`, OR delete fixtures + README refs).
- **Source:** `docs/principles-over-specifics-audit-2026-06-25.md` BUG 1; behavioral-collapse task 2026-06-25.
