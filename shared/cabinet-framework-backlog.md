...
- **Open (v1.1/v1.2):** (1) **CTO #3 context_slug NOT-NULL fix** — mirror writes `untagged` sentinel not NULL (hook/SQL edit + one-time backfill of existing NULL rows); (2) tagging-discipline layer (auto-inference + SOFT nudge, not hard enforcement per Captain D4); (3) dashboard view (source badge + project filter/untagged bucket + founder-action view + history).
...
- **TLS/DNS:** Cloudflare terminates public TLS for `proxy.refslund.ai` (+ audit/dashboard subdomains) → forwards to the origin VPS; **Caddy** (CTO architecture pick over nginx — simpler config for the internal-routing-only role, since Cloudflare terminates public TLS) is the LOCAL origin reverse-proxy routing to internal service ports (Spec 051 CTO#7 + §Topology; Spec 052 CTO#2). NOT Caddy-as-public-TLS (that is Cloudflare's job).
...
- **Dependencies:** Hetzner Frankfurt VPS provisioning (#191, Captain founder-action) is the ONLY gate. The CTO Hetzner-vs-Fly.io architecture ratification is **DONE** (Hetzner Frankfurt, captain-decisions 2026-05-25 07:13 UTC — GDPR EU-residency / no-US-parent / no third-country-transfer; Hetzner Cloud CPX, EU/Germany region). Deploys the FW-096 + FW-097 substrate.
