...
- **Neon** — Product database (schema, queries, migrations). **Config-layer server entry parked 2026-07-07** (doctor DEAD sweep): `NEON_API_KEY` was provisioned on no fleet env surface, so the server never once authenticated. Lane-CEO grants stay in `cabinet/mcp-scope.yml` (reserved-for-when-wired); restore recipe in `.mcp.json.mac-native` → `_neon_pending` once the Captain provisions the key into `cabinet/.env`. Until then, product-DB access is the direct connection string (`NEON_CONNECTION_STRING`).
