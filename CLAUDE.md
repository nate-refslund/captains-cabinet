...
- **Neon** — Product database (schema, queries, migrations). **Restored 2026-07-09** (Captain-call seal, resolution #9): the Captain provisioned `NEON_API_KEY` into `cabinet/.env` (600), the http server entry (`https://mcp.neon.tech/mcp`, env-based bearer auth) is live in BOTH config layers lockstep, live handshake verified (HTTP 200), doctor GREEN. Lane-CEO grants in `cabinet/mcp-scope.yml` (polads-ceo, stephie-ceo) now bind at boot. Direct `NEON_CONNECTION_STRING` access remains available alongside.
