...
## Model Routing
- **No single surface owns the model id, and the file this used to name disclaims it.** `instance/config/platform.yml` says so in its own words: *"The LLM model is NOT parsed from this file."* Where it actually lives, as of 2026-08-26: OFFICER models come from the launcher environment (`CABINET_MODEL`, defaulted in `start-officer-mac.sh` and `generate-instance.py`, and set per officer in the LaunchAgent). FRAMEWORK-INTERNAL dispatch pins its own model in code, separately, and on an older generation than the officers run — five pins across `run_action_lane.py`, `oauth_llm.py`, `actfirst_canary.py`, `onboarding/plan.py` and `onboarding/onboard.py`. Changing the model the outfit thinks with is therefore not one edit, and nothing in the tree makes those five agree. Officers run the orchestrator model at max effort; crews default to a cheaper model, escalated to orchestrator-grade for adversarial review, architecture, or security. Per-model prompting style: the cards in `.claude/model-cards/` — read the card for the model you are dispatching TO, not the one you are running on. Gotcha: single-quote any model id in a shell (a `[1m]` suffix globs).
