...
- **Evidence-driven graduated autonomy.** Under the default `guardian` posture the reversible classes act from day one (with a registered inverse and a 48h undo handle) and everything else is proposed to you; under `earn_up` every consequential action starts as a proposal. Approvals, edits, skips, and machine-verified outcomes land on an append-only consequence ledger; per-action-class graduation math converts that evidence into earned autonomy — and revokes it on wrong verdicts or fabrication.
...
- **Propose-first for the unproven, act-first for the reversible:** under the default `guardian` posture, unmeasured earn-up classes are propose-only by construction, while the reversible/undo-backed classes act from day one and drop back to propose-only on demotion evidence. `earn_up` makes everything propose-only.
