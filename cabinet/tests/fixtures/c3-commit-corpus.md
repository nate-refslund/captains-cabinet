...
| V11 | `` git commit -m "feat: x" `head -n 5 f` `` | N | Opus MEDIUM#4: `-n` inside a backtick command-sub is dropped (the sub BODY is stripped, like a quoted span) → no_verify=N |
| V12 | `` git commit `true` --no-verify -m "x" `` | **Y** | Opus HIGH#5 regression-guard: a real `--no-verify` AFTER a backtick sub must survive — strip the sub BODY, do NOT split on the backtick (splitting severed + missed it) |
| V13 | `` git commit -m "x" `foo --no-verify` `` | N | the `--no-verify` is an arg to `foo` INSIDE the sub (not passed to git commit) → correctly dropped with the sub body |
