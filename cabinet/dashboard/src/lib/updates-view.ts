/**
 * updates-view.ts — the update path's shape and its wording, and nothing a
 * browser cannot hold.
 *
 * WHY THIS FILE IS SEPARATE FROM `updates.ts`, which is the only reason it
 * exists. The home card is a client component and it needs three things from
 * the update path: the status TYPE, the gate that decides whether Apply is
 * offered, and the sentence a refusal becomes. `updates.ts` also carries the
 * exec seam — `child_process` — and a client component that imports one symbol
 * from a module drags the WHOLE module into the browser graph. Measured
 * 2026-09-10 on the first real staged build: `next build` failed outright with
 *
 ...
 *
 * — so the dashboard had not been buildable since the update path landed, and
 * nothing noticed because no gate anywhere ran `next build`. The drill's P7
 * now runs the real build, which is the sensor this file's absence needed.
 *
 * THE RULE, stated once: everything here is PURE — types, regexes, and
 * functions over a status object. No node import may enter this file, or the
 * card stops building again. `updates.ts` re-exports all of it, so a server
 * module still imports one name from one place.
 */
