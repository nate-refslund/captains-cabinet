...
# Resolve CABINET_ROOT from this hook's location (hooks/ → scripts/ → cabinet/ → repo root)
...
# ============================================================
# 3-7. TYPED POLICY ENGINE (replaces bash-regex sections 3-7)
# ============================================================
# Sections 3 (prohibited actions), 4 (codebase ownership),
# 5 (constitution protection), 6 (Layer 1 gate), and
# 7 (CI green gate) are now handled by the typed policy engine.
# The engine uses proper shell parsing instead of regex to detect
# prohibited commands, and YAML-defined policies for path blocks
# and ownership rules.
#
# History: sections 3-7 contained ~1200 lines of regex (v3.7.2,
# 7 adversary review cycles). The typed engine handles all bypass
# patterns through shell command parsing (~300 lines of Python).
# See framework/policies/ and presets/work/policies/ for rules.
