...
if has_capability "deploys_code" && [ "$TOOL_NAME" = "Bash" ]; then
  ...
  #   git push https://x-access-token:$PAT@github.com/owner/product-repo main
  ...
  elif echo "$CMD" | grep -qE '(^|[^a-z0-9_-])git push[[:space:]]+(.*[[:space:]:])?(refs/heads/)?(main|master)([[:space:];]|$)|gh pr merge|pulls/[0-9]+/merge|curl.*pulls/[0-9]+/merge'; then
