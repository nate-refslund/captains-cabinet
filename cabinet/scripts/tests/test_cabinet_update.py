...
_DASH_LIB = _SCRIPTS_DIR / "lib" / "dashboard.sh"
_STUB_DASH = _SCRIPTS_DIR / "drills" / "lib" / "stub_dashboard.py"
...
def make_install(tmp_path: Path, *, source_commit: str = OLD_SHA,
    ...
    # The REAL dashboard library. Every installed Cabinet has one, and two legs
    # of the updater now go through it: the staged build materialises the
    # install's dependencies with `cabinet_dash_link_modules` (A5.18) and the
    # door is probed with `cabinet_dash_state` before the first write (A5.19).
    # A fixture without it would exercise neither, and both of them are the
    # kind of thing that only ever fails on a real box.
    shutil.copy2(_DASH_LIB, root / "cabinet" / "scripts" / "lib" / "dashboard.sh")
...
def test_the_restarted_dashboard_does_not_inherit_the_updater_lock(tmp_path):
    ...
    A5.19 CHANGED WHAT THIS ARM HAS TO SET UP, and the change is the amendment
    working. A rollback now restarts only when one of this cabinet's own
    dashboards was answering before it began, so a rollback on a box with an
    empty door restarts nothing and this arm would measure a restart that never
    happened. So the box IS serving here, through the drill's health stub on a
    real port, and the fixture library APPENDS its stand-in restart to the real
    one rather than replacing the file: the door probe needs the real
    `cabinet_dash_state`, and a fixture that overrides more than the one
    function it stands in for is a fixture testing itself."""
    root = make_install(tmp_path)
    make_bundle(tmp_path, root, NEW_SHA, extra={"cabinet/scripts/hello.sh": "echo hello v2\n"})
    assert run_updater(root, "apply", "--bundle", NEW_SHA,
                       "--skip-rebuild", "--skip-restart").returncode == 0
    ...
    port = _free_port()
    stub_state = tmp_path / "stub.json"
    stamp = tmp_path / "stamp.txt"
    stamp.write_text(OLD_SHA + "\n", encoding="utf-8")
    boot = subprocess.run(
        [_PY, str(_STUB_DASH), "restart", "--port", str(port), "--stamp-file", str(stamp),
         "--state-file", str(stub_state), "--service", _dash_service()],
        capture_output=True, text=True, timeout=60)
    assert boot.returncode == 0, (boot.stdout, boot.stderr)

    pid_file = root / ".updates" / "stub-child.pid"
    dash_lib = root / "cabinet/scripts/lib/dashboard.sh"
    dash_lib.write_text(dash_lib.read_text(encoding="utf-8") + f"""
...
""", encoding="utf-8")
    try:
        result = run_updater(root, "rollback", timeout=90,
                             env_extra={"CABINET_DASHBOARD_PORT": str(port)})
        assert result.returncode == 0, (result.returncode, result.stderr)
        assert pid_file.is_file(), (
            "the stub restart never ran: %s"
            % (root / ".updates" / "update.log").read_text(encoding="utf-8")[-1500:])
        ...
        try:
            ...
            pass
        subprocess.run([_PY, str(_STUB_DASH), "stop", "--state-file", str(stub_state)],
                       capture_output=True)
...
# ===========================================================================
# 12. the staged rebuild is real and portable (A5.18), and an update restores
#     the process state it found (A5.19)
#
# Both amendments were ruled after the FIRST real apply on the Captain's
# installed Cabinet, 2026-09-10. It refused nothing, staged the tree, and then
# died inside `next build`:
#
#     Symlink [project]/node_modules is invalid, it points out of the
#     filesystem root
#
# — because the stage reused the install's dependencies through a symlink that
# left the staged project. The rollback was correct and is untouched here. On
# the way out it started an unsupervised dashboard on a door where nothing had
# been listening, which is the second amendment.
#
# WHY THE DRILL DID NOT CATCH EITHER. Its P7 passed `--skip-rebuild` on every
# apply, so the staged build was the one leg of the update path nothing walked.
# These arms are the fast half of closing that; the drill's REAL rebuild is the
# slow half, and it is the one that runs a genuine `next build`.
# ===========================================================================
...
def _dash_service() -> str:
    """The identity marker, read from the ONE line that declares it — never a
    copy here, which would go on passing after the real one changed."""
    for line in _DASH_LIB.read_text(encoding="utf-8").splitlines():
        if line.startswith("CABINET_DASH_SERVICE="):
            return line.split("=", 1)[1].strip().strip('"')
    raise AssertionError("cabinet/scripts/lib/dashboard.sh declares no CABINET_DASH_SERVICE")
...
#: A dashboard directory in the INSTALL: the dependency tree an update reuses,
#: plus one file inside it whose bytes and inode the stage must not disturb.
def plant_dependencies(root: Path, *, packages: int = 3) -> Path:
    modules = root / "cabinet" / "dashboard" / "node_modules"
    for n in range(packages):
        _write(modules / f"pkg{n}" / "index.js", f"module.exports = {n}\n")
        _write(modules / f"pkg{n}" / "package.json", '{"name": "pkg%d"}\n' % n)
    (modules / ".bin").mkdir(parents=True, exist_ok=True)
    # A relative symlink INSIDE the tree — `.bin` is a directory of them, and a
    # mechanism that flattened them into copies would break every tool call.
    link = modules / ".bin" / "pkg0"
    if not link.is_symlink():
        os.symlink("../pkg0/index.js", link)
    return modules


#: The dashboard half of a bundle: the lockfile the install will match on, and
#: one changed dashboard file — WITHOUT which the plan never touches
#: `cabinet/dashboard/` and the rebuild is not even attempted.
DASHBOARD_SHIPPED = {
    "cabinet/dashboard/package-lock.json": '{"name": "d", "lockfileVersion": 3}\n',
    "cabinet/dashboard/next.config.ts": "export default {}\n",
}


#: What the stage looks like from INSIDE it, written by the build seam. A file
#: rather than a shell one-liner because it has to survive the degenerate ends
#: — no node_modules at all, or one that is a symlink — and REPORT them, not
#: crash on them: an arm whose probe dies leaves the failure saying "the build
#: failed", which is true of a hundred things and names none of them.
_STAGE_PROBE = r"""
import json, os, sys

out = {"exists": os.path.lexists("node_modules"),
       "is_symlink": os.path.islink("node_modules"),
       "is_dir": os.path.isdir("node_modules") and not os.path.islink("node_modules")}
if out["is_dir"]:
    sample = os.path.join("node_modules", "pkg0", "index.js")
    out["names"] = sorted(os.listdir("node_modules"))
    out["inode"] = os.stat(sample).st_ino
    out["nlink"] = os.stat(os.path.join("node_modules", "pkg1", "index.js")).st_nlink
    out["body"] = open(sample).read()
    binlink = os.path.join("node_modules", ".bin", "pkg0")
    out["bin_is_symlink"] = os.path.islink(binlink)
    out["bin_resolves"] = os.path.exists(binlink)
json.dump(out, open(sys.argv[1], "w"))
os.makedirs(".next", exist_ok=True)
open(os.path.join(".next", "marker"), "w").write("built")
"""


def _stage_probe(tmp_path: Path) -> tuple[str, Path]:
    """The build-seam command that photographs the stage, and where it lands."""
    script = tmp_path / "stage-probe.py"
    script.write_text(_STAGE_PROBE, encoding="utf-8")
    report = tmp_path / "stage-report.json"
    return f"{_PY} {script} {report}", report


def test_the_stage_gets_a_hardlink_tree_and_never_a_symlinked_node_modules(tmp_path):
    """A5.18(1), and the arm is RED on the bytes that met the Captain.

    The build seam runs INSIDE the stage, so it is where the stage can be
    examined exactly as the real build sees it. On ae687861 what it sees is
    either a symlink out of the project — the thing the bundler refused, "it
    points out of the filesystem root" — or, under the seam, no dependency tree
    at all, because the link sat on the far side of the branch the seam takes.
    Both are the same defect: the stage's dependencies were never IN the
    stage."""
    root = make_install(tmp_path)
    plant_dependencies(root)
    make_bundle(tmp_path, root, NEW_SHA, extra=dict(DASHBOARD_SHIPPED))
    probe, report = _stage_probe(tmp_path)

    result = run_updater(root, "apply", "--bundle", NEW_SHA, "--skip-restart",
                         env_extra={"CABINET_UPDATE_TEST_BUILD_CMD": probe})
    assert report.is_file(), (
        "the build seam never ran, so nothing about the stage was measured: %s"
        % (result.stderr,))
    seen = json.loads(report.read_text(encoding="utf-8"))

    assert not seen["is_symlink"], (
        "the stage's node_modules is a SYMLINK. That is the byte-for-byte defect the "
        "Captain's first real apply died on: 'Symlink [project]/node_modules is invalid, "
        "it points out of the filesystem root' (A5.18)")
    assert seen["is_dir"], (
        "the stage has no dependency tree of its own (%r), so a real build in it would "
        "either fail outright or pay a full install per update" % (seen,))
    assert result.returncode == 0, (result.returncode, result.stdout, result.stderr,
                                    (root / ".updates" / "update.log").read_text())

    live = root / "cabinet" / "dashboard" / "node_modules"
    assert seen["names"] == sorted(os.listdir(live)), seen["names"]
    assert seen["body"] == "module.exports = 0\n", seen
    # HARDLINK, not a copy: the same inode is what makes reuse free and what
    # makes dropping the stage harmless.
    assert seen["inode"] == os.stat(live / "pkg0" / "index.js").st_ino, (
        "the stage's dependency tree is a COPY, not a hardlink tree — every update "
        "would cost the operator a second copy of it")
    assert seen["bin_is_symlink"] and seen["bin_resolves"], (
        "an internal .bin symlink did not survive the link, so every tool call "
        "through it would fail in the stage: %r" % (seen,))


def test_dropping_the_stage_leaves_the_installs_dependency_tree_intact(tmp_path):
    """A5.18(1), the removal property — the one hardlinks could get wrong.

    Sharing inodes is only safe if nothing writes THROUGH them. The stage is
    removed at the end of every apply; if the mechanism were a move, or if the
    removal followed the links, the install would lose its dependencies and the
    NEXT update would have nothing to reuse.

    THE LINK COUNT IS ALSO READ FROM INSIDE THE STAGE, not only after it, and
    that is what makes this arm red on ae687861 rather than vacuously green: a
    symlinked stage leaves the install's files at one link the whole way
    through, so "the tree survived" would be true of a stage that never shared
    it in the first place."""
    root = make_install(tmp_path)
    live = plant_dependencies(root)
    sampled = live / "pkg1" / "index.js"
    before_inode = os.stat(sampled).st_ino
    before_body = sampled.read_text(encoding="utf-8")
    before_names = sorted(os.listdir(live))
    before_links = os.stat(sampled).st_nlink
    assert before_links == 1, before_links

    make_bundle(tmp_path, root, NEW_SHA, extra=dict(DASHBOARD_SHIPPED))
    probe, report = _stage_probe(tmp_path)
    result = run_updater(
        root, "apply", "--bundle", NEW_SHA, "--skip-restart",
        env_extra={"CABINET_UPDATE_TEST_BUILD_CMD": probe})
    assert result.returncode == 0, (result.stdout, result.stderr)
    staged = json.loads(report.read_text(encoding="utf-8"))
    assert staged.get("nlink", 1) >= 2, (
        "while the stage existed the install's dependency file had %r link(s): the "
        "stage never shared the tree, so this arm would pass on a mechanism that "
        "handed the build nothing at all" % (staged.get("nlink"),))

    # The stage is gone (A5.14) and the install's tree is exactly as it was.
    stage = root / ".updates" / "stage" / NEW_SHA
    assert not stage.exists(), "the staged tree outlived its apply"
    assert sorted(os.listdir(live)) == before_names
    assert sampled.read_text(encoding="utf-8") == before_body
    assert os.stat(sampled).st_ino == before_inode
    # The stage's names are unlinked, so the link count comes back down.
    assert os.stat(sampled).st_nlink == before_links, (
        "the install's dependency file still carries a link from a stage that is "
        "supposed to be gone")


def test_a_rollback_does_not_leave_a_dashboard_on_a_door_that_was_empty(tmp_path):
    """A5.19, and this is the measurement, not a theory.

    On the Captain's box nothing was listening, the apply failed at the build,
    the rollback was correct — and it started an unsupervised dashboard he had
    not been running. The restart command here would START one, so a green arm
    means the updater DECLINED to run it."""
    root = make_install(tmp_path)
    plant_dependencies(root)
    make_bundle(tmp_path, root, NEW_SHA, extra=dict(DASHBOARD_SHIPPED))
    port = _free_port()
    stub_state = tmp_path / "stub.json"
    stamp = tmp_path / "stamp.txt"
    stamp.write_text(NEW_SHA + "\n", encoding="utf-8")
    assert not _listening(port), "the port was already busy before the arm began"

    restart = (f"{_PY} {_STUB_DASH} restart --port {port} --stamp-file {stamp} "
               f"--state-file {stub_state} --service {_dash_service()}")
    try:
        result = run_updater(
            root, "apply", "--bundle", NEW_SHA,
            env_extra={
                "CABINET_DASHBOARD_PORT": str(port),
                "CABINET_UPDATE_TEST_BUILD_CMD": "exit 1",
                "CABINET_UPDATE_TEST_RESTART_CMD": restart,
            })
        assert result.returncode == 1, (result.returncode, result.stdout, result.stderr)
        assert state(root)["phase"] == "rolled_back", state(root)
        assert not stub_state.is_file(), (
            "the rollback started a dashboard on a door where nothing had been "
            "listening — the operator asked for an update, did not get one, and was "
            "left running a process he did not have (A5.19)")
        assert not _listening(port), "something is listening on a door that was free"
        log = (root / ".updates" / "update.log").read_text(encoding="utf-8")
        assert "restart after rollback: NOT started" in log, log[-2000:]
    finally:
        subprocess.run([_PY, str(_STUB_DASH), "stop", "--state-file", str(stub_state)],
                       capture_output=True)


def test_a_rollback_puts_back_the_dashboard_it_found(tmp_path):
    """A5.19's other half, which is what keeps it from being "never restart".

    A box that WAS serving must still be serving when the update it could not
    take is put back. The stub's restart replaces the process, so a new pid on
    the same door is the proof that the restart ran rather than that the old
    one was merely left alone."""
    root = make_install(tmp_path)
    plant_dependencies(root)
    make_bundle(tmp_path, root, NEW_SHA, extra=dict(DASHBOARD_SHIPPED))
    port = _free_port()
    ...
    stamp = tmp_path / "stamp.txt"
    stamp.write_text(OLD_SHA + "\n", encoding="utf-8")
    service = _dash_service()
    restart = (f"{_PY} {_STUB_DASH} restart --port {port} --stamp-file {stamp} "
               f"--state-file {stub_state} --service {service}")
    try:
        boot = subprocess.run(
            [_PY, str(_STUB_DASH), "restart", "--port", str(port), "--stamp-file",
             str(stamp), "--state-file", str(stub_state), "--service", service],
            capture_output=True, text=True, timeout=60)
        assert boot.returncode == 0, (boot.stdout, boot.stderr)
        pid_before = json.loads(stub_state.read_text(encoding="utf-8"))["pid"]
        ...
        assert _listening(port), (
            "the box was serving before the update and is not serving after it — a "
            "rollback that takes the dashboard down is worse than the update it refused")
        pid_after = json.loads(stub_state.read_text(encoding="utf-8"))["pid"]
        assert pid_after != pid_before, (
            "the dashboard on the door is the SAME process, so the rollback never "
            "restarted it and it is still serving the bytes the update replaced")
        log = (root / ".updates" / "update.log").read_text(encoding="utf-8")
        assert "so it is put back" in log, log[-2000:]
    finally:
        subprocess.run([_PY, str(_STUB_DASH), "stop", "--state-file", str(stub_state)],
                       capture_output=True)


def test_the_door_is_read_before_the_first_write(tmp_path):
    """A5.19's ordering, which is the whole of its correctness.

    "What was running before" is only answerable before this script has changed
    anything — the resume-restore, the plan, the write and the build all move
    the box. The log line therefore has to come out ahead of them, and this arm
    reads the log in order rather than trusting the source."""
    root = make_install(tmp_path)
    make_bundle(tmp_path, root, NEW_SHA)
    result = run_updater(root, "apply", "--bundle", NEW_SHA, "--skip-rebuild", "--skip-restart")
    assert result.returncode == 0, (result.stdout, result.stderr)
    lines = [ln for ln in (root / ".updates" / "update.log").read_text(encoding="utf-8").splitlines()
             if ln.strip()]
    doors = [i for i, ln in enumerate(lines) if "door before this apply:" in ln]
    assert doors, "the updater never recorded what was on the door before it began"
    writes = [i for i, ln in enumerate(lines)
              if "applied " in ln or "build:" in ln or "gate:" in ln or "restart" in ln]
    assert not writes or doors[0] < min(writes), (
        "the door was read AFTER this run had already changed the box:\n%s"
        % "\n".join(lines[:12]))


def test_the_dependency_mechanism_is_the_shipped_one(tmp_path):
    """Structural, and it is the arm that keeps the two callers from drifting.

    The updater and the acceptance drill must materialise the stage's
    dependencies by the SAME mechanism, or the drill proves a copy of the
    mechanism rather than the mechanism. Both call
    `cabinet_dash_link_modules`, which lives in the dashboard library exactly
    once; and neither may symlink a `node_modules` anywhere, which is the
    defect being removed."""
    lib = _DASH_LIB.read_text(encoding="utf-8")
    assert lib.count("cabinet_dash_link_modules() {") == 1, "the mechanism is not declared once"
    for name in ("pax -rwl", "--link-dest", "cp -R"):
        assert name in lib, "the mechanism lost its %r fallback" % name

    updater = (_SCRIPTS_DIR / "cabinet-update.sh").read_text(encoding="utf-8")
    drill = (_SCRIPTS_DIR / "drills" / "one-responsibility.sh").read_text(encoding="utf-8")
    for who, text in (("the updater", updater), ("the drill", drill)):
        assert "cabinet_dash_link_modules" in text, (
            "%s does not call the shipped mechanism" % who)
