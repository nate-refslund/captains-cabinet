...
def _eval_authority_matrix(
    ...
    2026-07-03) against the posture-selected verdicts table: a graduated cell
    can now resolve auto (returns None) — consumed in SHADOW only until the
    Captain-gated enforcement flip. With no posture config the resolution and
    every block string are byte-identical to the guardian/legacy gate. See
    the section headers for the fail-safe contract.
    """
    ...
    # 3. Read F's PRECOMPUTED cell state; resolve against the posture table.
    ...
    # 4. Allow-set {auto, notify_after} (D4). Everything else blocks: in the
    #    guardian/A0 world `state` is "unmeasured" and every non-ceiling cell
    #    lands here as propose_only, byte-identical. notify_after (sovereign
    #    tables only — guardian carries none) allows AND emits the tell: the
    #    gate returns None so no acted_row exists; the org_event is what the
    #    digest renders. A standing_grant verdict reaching this step is
    #    MISPLACED (only legal on a posture ceiling row, handled above) —
