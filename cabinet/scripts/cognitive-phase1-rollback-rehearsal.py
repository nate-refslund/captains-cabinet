#!/usr/bin/env python3
"""Rehearse the COG-1 CODE inverse in a disposable git worktree.

Phase-local twin of cabinet/scripts/cognitive-phase0-rollback-rehearsal.py
(§12.4; the Phase-0 instance stays untouched — §10.3). The landed tree is never
modified. The rehearsal removes additive COG-1 files, restores the extended
files from the pinned baseline (which also removes the COG-1 temporary_allowances
rows via the contract yml restore), retains the append-only operative rows with
simulated rollback notes, proves the remaining diff is exactly those two ledgers,
and runs the pre-phase safety/compatibility gates against that inverse tree.

The RUNTIME inverses (authority flip-back; disarm/enable capture) are rehearsed
separately against a scratch pointer and the W2 ephemeral PostgreSQL 17 harness
in cabinet/scripts/tests/test_cog1_cutover.py (the manifest names both).
"""
...
ROLLBACK_SUFFIX = (
    " | ROLLBACK-REHEARSAL 2026-07-20: simulated supersession; "
    "COG-1 implementation bytes removed and history retained."
)

# Byte-identical to the A13 assertion in verify-cognitive-phase1.sh's heredoc
...
def main() -> int:
    ...
    for rel in manifest["must_remain_unchanged"]:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"COG-1 modified protected surface: {rel}")

    scratch = Path(tempfile.mkdtemp(prefix="cog1-rollback-")) / "tree"
    try:
        ...
        # assertion verify-cognitive-phase1.sh runs on the committed tree, here on
        # the inverse tree AFTER the append-only notes. Unconditional (structural),
        # so --skip-compatibility keeps it.
        ...
        print("COG-1 rollback rehearsal: PASS (only append-only operative history remains)")
        return 0
    finally:
        if scratch.exists():
            subprocess.run(
