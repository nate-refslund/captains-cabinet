#!/usr/bin/env python3
"""Rehearse the COG-2 CODE inverse in a disposable git worktree.

Phase-local twin of cabinet/scripts/cognitive-phase1-rollback-rehearsal.py
(§12.4; the Phase-0/1 instances stay untouched — each phase owns its own
rehearsal). The landed tree is never modified. The rehearsal removes additive
COG-2 files, restores the extended files from the pinned baseline (which also
removes the COG-2 temporary_allowances rows via the contract yml restore),
...
Unlike COG-1, COG-2 is SHADOW-ONLY: there is no authority cutover to rehearse.
The three runtime inverses are safe by construction — the read pointer is `none`
all phase, the projection cache is a lossless rebuildable function of the
sources (§7.3), and cortex_ro is an additive idempotent SELECT-only role — so
this rehearsal covers the CODE inverse only (the manifest names the runtime
inverses; none needs a harness).

The A13 check below is the universal operative-ledger parity (ledger-id
uniqueness + plan_ids == set(ids)) — the SAME source the phase-0/1 gates run
via their `<<'PY'` heredocs, and verify-cognitive-phase2.sh carries the
byte-identical heredoc too. It is run here on the inverse tree; the rollback
test pins this A13_ASSERTION byte-identical against the verify script's heredoc
...
ROLLBACK_SUFFIX = (
    ...
    "COG-2 implementation bytes removed and history retained."
)
...
# This copy is byte-identical to verify-cognitive-phase2.sh's heredoc (the
...
def main() -> int:
    ...
    for rel in manifest["must_remain_unchanged"]:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"COG-2 modified protected surface: {rel}")

    scratch = Path(tempfile.mkdtemp(prefix="cog2-rollback-")) / "tree"
    try:
        ...
        # universal assertion the phase-0/1 gates run, here on the inverse tree
        ...
        print("COG-2 rollback rehearsal: PASS (only append-only operative history remains)")
        return 0
    finally:
        if scratch.exists():
            subprocess.run(
