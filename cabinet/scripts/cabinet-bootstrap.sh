...
LOCK_FD=9
FRAMEWORK_REPO_URL="${CABINET_FRAMEWORK_REPO_URL:-https://github.com/YOUR-ORG/captains-cabinet}"
...
step_init_instance_dirs() {
  ...
  # instance/config/projects/ (e.g. the original project's YAML). New cabinet must
...
step_generate_docker_compose() {
  ...
  # FW-082 hotfix-5 (CoS field report msg 2232 / Captain push to alive): emit
  # an actual officers-${CABINET_SLUG} service (not a commented-out template)
  # so first compose-up brings the CEO officer alive. Bot mode determines
  # which officers auto-start: single_ceo → just the CEO archetype (cos by
  # default); multi_officer → all archetypes from preset.yml.
  local _bot_mode="multi_officer"
  local _ceo_officer="cos"
  local _preset_yml="$CABINET_ROOT/presets/$PRESET_SLUG/preset.yml"
  if [ -f "$_preset_yml" ]; then
    local _raw_mode
    _raw_mode=$(grep -E '^[[:space:]]*telegram_bot_mode:' "$_preset_yml" 2>/dev/null | head -1 \
  ...
  # Determine AUTO_START_OFFICERS list (consumed by entrypoint.sh).
  # single_ceo: just the CEO archetype (default cos).
  # multi_officer: all archetypes listed under agent_archetypes in preset.yml.
  local _auto_start=""
  if [ "$_bot_mode" = "single_ceo" ]; then
    _auto_start="$_ceo_officer"
  else
    if [ -f "$_preset_yml" ]; then
      _auto_start=$(awk '
        /^agent_archetypes:/ {in_block=1; next}
        in_block && /^[a-zA-Z]/ {in_block=0}
        in_block && /^[[:space:]]+- [a-z]/ {
          gsub(/^[[:space:]]+- /, "")
          gsub(/[[:space:]].*$/, "")
          if ($0 ~ /^[a-z][a-z0-9-]*$/) print $0
        }
      ' "$_preset_yml" | paste -sd, -)
    fi
    # Fallback to canonical 5-officer roster if parse fails.
    [ -z "$_auto_start" ] && _auto_start="cos,cto,cpo,cro,coo"
  fi

  {
    cat <<COMPOSE
# docker-compose.yml — Cabinet: ${CABINET_SLUG}
# Generated by cabinet-bootstrap.sh (FW-082 + hotfix-5) at $(date -u +%Y-%m-%dT%H:%M:%SZ)
# Preset: ${PRESET_SLUG}
# Bot mode: ${_bot_mode}
# Auto-start officers: ${_auto_start}
#
...
#
# CAPTAIN ACTION (one-time per cabinet):
#   1. Add TELEGRAM_<UPPER_PROJECT_SLUG>_CEO_TOKEN to ./cabinet/.env (single_ceo)
#      OR per-officer TELEGRAM_<OFFICER>_TOKEN (multi_officer).
#   2. docker compose up -d
#   3. Officers auto-start in tmux per AUTO_START_OFFICERS env.
...
services:
  ...
  cabinet-mcp-${CABINET_SLUG}:
    # FW-082 hotfix-4: build locally from framework Dockerfile so first-spawn
    # works without an upstream registry. Operator can pre-tag for cache hit:
    ...
    environment:
      ...
      - REDIS_PORT=6379
      # FW-082 hotfix-5: CABINET_ROOT points at the actual spawned-cabinet
      # directory (was incorrectly /opt/<slug>-cabinet which doesn't exist).
      - CABINET_ROOT=${cabinet_dir}
    env_file:
      - ./cabinet/.env
    volumes:
      # Mount /opt/founders-cabinet (parent of spawned-cabinets/) so the
      # spawned cabinet's framework copy + entrypoint hardcoded paths both
      # resolve. The spawned cabinet's own state lives at \${cabinet_dir}.
      - /opt/founders-cabinet:/opt/founders-cabinet
    networks:
      - cabinet-${CABINET_SLUG}
    depends_on:
      redis-${CABINET_SLUG}:
        condition: service_healthy
    command: ["python3", "${cabinet_dir}/cabinet/mcp-server/server.py"]
    ports:
      - "7471"  # Expose for peer HTTP — bind specific host port in overlay if needed

  # ----------------------------------------------------------------
  # Officers container — runs all archetypes via tmux + supervisor.
  # FW-082 hotfix-5: emitted directly (not a commented template) so first
  # compose-up brings officers alive. AUTO_START_OFFICERS env is parsed by
  # entrypoint.sh to spawn each officer in its own tmux window.
  # ----------------------------------------------------------------
  officers-${CABINET_SLUG}:
    build:
      context: /opt/founders-cabinet/cabinet
      dockerfile: Dockerfile.officer
    image: officers-${CABINET_SLUG}:latest
    container_name: officers-${CABINET_SLUG}
    restart: unless-stopped
    environment:
      - CABINET_ID=${CABINET_SLUG}
      - CABINET_MODE=multi
      - REDIS_HOST=redis-${CABINET_SLUG}
      - REDIS_PORT=6379
      - REDIS_URL=redis://redis-${CABINET_SLUG}:6379
      - CABINET_ROOT=${cabinet_dir}
      # AUTO_START_OFFICERS is consumed by entrypoint.sh: comma-separated
      # list of "officer[:project]" pairs. Each entry gets its own tmux
      # window. single_ceo bot mode → just the CEO; multi_officer → full roster.
      - AUTO_START_OFFICERS=${_auto_start}
      - CLAUDE_CODE_EFFORT_LEVEL=max
    env_file:
      - ./cabinet/.env
    volumes:
      - /opt/founders-cabinet:/opt/founders-cabinet
      - claude-auth-${slug_under}:/home/cabinet/.claude
      - channels-state-${slug_under}:/home/cabinet/.claude-channels
      # Spec 035 Phase A: host-agent socket (read-only).
      - /run/cabinet:/run/cabinet:ro
    working_dir: ${cabinet_dir}
    stdin_open: true
    tty: true
    networks:
      - cabinet-${CABINET_SLUG}
...
volumes:
  ...
  claude-auth-${slug_under}:
    name: claude-auth-${slug_under}
  channels-state-${slug_under}:
    name: channels-state-${slug_under}
...
COMPOSE
  ...
  info "docker-compose.yml generated: $compose_file (bot_mode=$_bot_mode, auto_start=$_auto_start)"
}
...
# FW-082 hotfix-5: docker-compose.yml is always regenerated on re-run. The
# generated file is a pure overwrite from template; template improvements
# (e.g. hotfix-5 single_ceo officers block + path fixes) need to backport
# to existing spawned cabinets without manual state-file surgery.
if [ -f "$STATE_FILE" ] && grep -qF "docker-compose" "$STATE_FILE" 2>/dev/null; then
  if [ "$DRY_RUN" != "1" ]; then
    grep -vF "docker-compose" "$STATE_FILE" > "${STATE_FILE}.tmp" && mv "${STATE_FILE}.tmp" "$STATE_FILE"
    info "docker-compose entry stripped from state — will regenerate"
  fi
fi
