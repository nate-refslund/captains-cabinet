...
DIGEST_JSON="$(python3 "$ORG" digest publish-sanitized \
  ...
  --content "Sensed is legacy. Contact nate@example.com. Secret sk-testsecret123. See https://example.com/private." \
...
if grep -qiE 'Sensed|nate@example.com|sk-testsecret123|https://example.com' "$DIGEST_PATH"; then
