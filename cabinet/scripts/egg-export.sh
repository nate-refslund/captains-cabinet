...
# --- shipped-ignored keep-list (2026-07-26) -------------------------------------
# The egg has no .git, so nothing in it can tell "shipped" from "written by this
# deployment". null-hatch.sh's gitless staging path re-derives that distinction
# from the tree's own .gitignore — and gets it WRONG for force-tracked paths:
# `git ls-files --others --ignored` in a fresh index lists every ignored path
# regardless of whether the source repo force-tracks it, so shipped content that
# happens to match .gitignore (the dashboard officers pages, memory/tier3
# structure, shared/interfaces/deployment-status.md) was deleted from the
# clean-room sandbox while `git archive HEAD` kept it. Same tree, two verdicts.
#
# The tree cannot answer this, so SHIP THE ANSWER: the set of paths this export
# carries that a fresh clone's .gitignore would call ignorable. Written AFTER the
# manifest pass and filtered to what actually survived it, so it never names a
# path the egg does not have. Consumers treat an ABSENT list as "keep nothing
# extra" — a plain git work tree therefore behaves exactly as before.
SHIPPED_IGNORED_REL="cabinet/scripts/shipped-ignored-paths.txt"
{
  echo "# Paths this export ships that a fresh clone's .gitignore would ignore."
  echo "# Generated by cabinet/scripts/egg-export.sh from"
  echo "#   git ls-files -c -i --exclude-standard  (at $SRC_COMMIT),"
  echo "# filtered to paths present after the packaging pass. Consumed by"
  echo "# cabinet/scripts/null-hatch.sh so its gitless staging keeps shipped"
  echo "# content and still prunes deployment-local state. One path per line."
  git -C "$REPO_ROOT" -c core.quotePath=false ls-files -c -i --exclude-standard \
    | while IFS= read -r _rel; do
        [ -n "$_rel" ] || continue
        [ -f "$OUT/$_rel" ] || continue
        printf '%s\n' "$_rel"
      done
} > "$OUT/$SHIPPED_IGNORED_REL"
SHIPPED_IGNORED_COUNT="$(grep -cv '^#' "$OUT/$SHIPPED_IGNORED_REL" || true)"
...
echo "  shipped-ignored: $SHIPPED_IGNORED_COUNT path(s) listed in $SHIPPED_IGNORED_REL"
