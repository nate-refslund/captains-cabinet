...
def _poison_new_modules(monkeypatch):
    ...
    for name in ("posture", "grants", "needs", "matrix"):
...
def test_sovereign_auto_without_inverse_proposes_and_files_capability_need(
    ...
    # investigation_run: reversible ⇒ sovereign auto at unmeasured, but the
    # kind has no registered inverse ⇒ propose + kind=capability need (D10)
    root = tmp_path / "root"
    monkeypatch.setenv("CABINET_ROOT", str(root))
    monkeypatch.setenv("CABINET_NEEDS_WIRED", "1")
    out = _drive_main(monkeypatch, proposals=[_card("investigation_run")],
                      pctx=_pctx(posture="sovereign"))
    assert out["delivers"] == [] and len(out["tgs"]) == 1
    ...
    assert any(row["kind"] == "capability"
               and row["action_type"] == "investigation_run" for row in rows)
...
def test_sovereign_capability_need_dark_without_wire(tmp_path, monkeypatch):
    ...
    _drive_main(monkeypatch, proposals=[_card("investigation_run")],
                pctx=_pctx(posture="sovereign"))
...
def test_route_verdict_sovereign_selects_posture_table():
    ...
    v, _, _ = r._route_verdict(ctx, _card("investigation_run"), "investigation_run")
    assert v == "auto"
