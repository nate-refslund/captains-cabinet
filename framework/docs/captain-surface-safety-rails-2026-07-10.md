...
|-------------------------------------------------------|---------|
| `captain_replied_since(slug, queued_ts)` is `True`     | CANCEL — the captain handled the thread himself (the worked **Sofie case**) |
...
Regression test: `framework/frontdoor/tests/test_fire_gate_sofie.py` (the
worked Sofie case end-to-end at the chokepoint) +
...
- `withdrawal_of(id)` — why a draft is gone; `deliver_draft` consults it so a
  ...
  "expired or already sent" miss (the root of the Sofie confusion).
