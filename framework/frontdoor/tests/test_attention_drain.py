...
(cabinet:captain-attention:polads / :stephie, cabinet:frontdoor:intake). If
...
def test_slug_guard_accepts_valid_and_rejects_bad():
    assert attention_drain._valid_slug("polads")
    assert attention_drain._valid_slug("polads-ceo")
...
def test_urgency_maps_to_tier(urgency, expected):
    card = {"source": "polads-ceo", "urgency": urgency,
            "summary": "s", "body": "b", "ts": "2026-06-24T05:00:00Z"}
    item = attention_drain.card_to_item(card, project="polads")
    assert item is not None
    assert item["urgency_tier"] == expected


def test_card_to_item_shapes_a_valid_intake_item():
    card = {"source": "polads-ceo", "project": "polads", "urgency": "high",
    ...
    assert item["source"] == "officer:polads-ceo"
    assert item["kind"] == "captain-attention"
    assert item["payload"]["summary"] == "[polads] Sentry blocker"
    assert item["payload"]["detail"] == "13k events/24h"
    assert item["payload"]["project"] == "polads"
    assert item["payload"]["origin_officer"] == "polads-ceo"


def test_card_with_no_summary_is_unusable():
    card = {"source": "polads-ceo", "urgency": "high", "summary": "",
            "body": "b", "ts": "2026-06-24T06:00:00Z"}
    assert attention_drain.card_to_item(card, project="polads") is None
...
def isolated_keys(monkeypatch):
    ...
    assert source_stream not in (
        "cabinet:captain-attention:polads", "cabinet:captain-attention:stephie")
...
def test_drain_forwards_card_into_intake(isolated_keys):
    k = isolated_keys
    _push_card(k["source"], source="polads-ceo", urgency="high",
    ...
    assert it["payload"]["origin_officer"] == "polads-ceo"
...
def test_drain_uses_own_group_not_the_ceo_group(isolated_keys):
    ...
    k = isolated_keys
    _push_card(k["source"], source="polads-ceo", urgency="low",
...
# 'PolAds PROD fire' card's summary bled onto the wrong entry id and one card was
...
_MULTILINE_BODY = (
    ...
    "3) The open lane work is outcome-polads-003 + the Sentry fire."
)
...
def test_multiline_bodies_do_not_scramble_id_content_alignment(isolated_keys):
    ...
    for summ, urg in expected.items():
        _push_card(k["source"], source="polads-ceo", urgency=urg,
