...
evidence-overlap check failed to match. Canonical ids are LOWERCASE by
contract (LLM case drift + macOS case-insensitive FS)."""
...
def test_url_normalization_drift_forms_overlap():
    base = canonical_refs(["https://polads.eu/portal"])
    assert canonical_refs(["see https://polads.eu/portal."]) & base
    assert canonical_refs(["https://polads.eu/portal/"]) & base
    assert canonical_refs(["HTTPS://POLADS.EU/portal"]) & base
    # pathless URL with sentence dot
    assert canonical_refs(["see https://polads.eu."]) & canonical_refs(["https://polads.eu"])


def test_monday_spellings_from_live_corpus():
    assert "monday:5098236573" in canonical_refs(["monday: 5098236573"])
    assert "monday:5098236573" in canonical_refs(["the ✔ Todos board `5098236573`"])
    assert "monday:5091706356" in canonical_refs(["Tasks board 5091706356 in AI Workspace"])
...
def test_path_grade_classifier():
    ...
    assert not path_grade("monday:5091706356")
...
def test_known_limitation_space_bearing_meeting_filenames():
    ...
    c = canonical_refs(["2-Meetings/2026-07-07 - PolAds Scrum.md — cmt-abcdef123456 discussed"])
