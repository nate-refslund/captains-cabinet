...
def _isolated_root(monkeypatch, tmp_path):
    ...
    # RECONCILE 2026-07-05: kept both — a REAL no-posture deployment carries
    # the shipped floor at CABINET_ROOT, and main's un-stubbed read_cell_state
    # (CRIT-3) has graduation.evaluate load bars from it. A bare tmp root
    # starves that load → evaluate raises → fail-closed "demote" (correct
    # runtime behavior, wrong test premise). Ship the floor into the tmp tree
    # (posture config still ABSENT ⇒ guardian) so the parity goldens measure
    # posture-axis identity, not an artificially broken evidence plane.
...
# RECONCILE 2026-07-05: kept both — under main's ratified trust-inversion
# floor a reversible Edit at unmeasured is act_with_undo (ALLOW when the
...
def test_gate_reversible_allow_byte_identical(monkeypatch, env, _isolated_root):
    ...
    # guardian reversible@unmeasured = act_with_undo; with the inverse
    # registered the gate allows (None) — identically in both env states.
...
def test_gate_propose_string_byte_identical(monkeypatch, env, _isolated_root):
    ...
    assert result.startswith("PROPOSE-ONLY (deploy_nonprod, confidence=")
