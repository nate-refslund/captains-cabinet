...
6. **Fail closed, degrade honestly.** Unmeasured means propose-only *for the earn-up rows*, and always-gated for the six ceiling rows; since the earn-demotion ruling (2026-07-03/04) the reversible/undo-backed rows are instead trusted at unmeasured and demoted on evidence (§3) — fail-closed there means the demotion path, not a propose-only floor. Credit exhaustion halts self-improvement rather than skipping gates. Veto windows hold when the Captain is unreachable. Probes report could-not-observe rather than guessing.
