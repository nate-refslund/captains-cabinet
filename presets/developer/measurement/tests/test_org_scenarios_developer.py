"""Concrete org-scenario seed tests (five-officer developer-preset archetype).

Developer-preset twin of presets/work/measurement/tests/test_org_scenarios.py
(the developer preset is a flat copy of work per presets/README.md — the
measurement seed is copied unchanged). FILENAME DELIBERATELY DIFFERS from
the work twin: neither tests/ dir ships an __init__.py, so under repo-root
pytest collection (pytest.ini pins rootdir; bare `pytest` collects
presets/) a same-basename module in two directories is a collection-time
import collision — the rename is the fix, everything else mirrors the twin.
The seed-module namespace below is likewise developer_preset_seed_scenarios
so this suite proves the DEVELOPER seed loads even when work's suite loaded
first in the same session (register() overwrites idempotently).

Relocated lineage (from the work twin): framework/measurement keeps the
runner/registry MACHINERY tests; these tests exercise the CONCRETE scenario
seed content that ships with the developer preset (../scenarios/).

The seed is written for its INSTALL location — a developer-preset deployment
...
# Repo root: presets/developer/measurement/tests/<this file> -> up 4 dirs.
...
_SEED_DIR = Path(__file__).resolve().parents[1]  # presets/developer/measurement
...
def _install_and_load(install_root: Path) -> None:
    ...
    for f in sorted((measurement / "scenarios").glob("*.py")):
        if f.name.startswith("_"):
            continue
        module_name = f"developer_preset_seed_scenarios.{f.stem}"
