...
3. **Assess build & deploy.** `package.json` scripts; does the build succeed? Tests present and passing? Vercel config and required env vars (deploy state reads via REST + the probe rows, not an MCP); CI/CD and pre-commit hooks.
3b. **Walk the GitHub surface.** Through the declared `github` MCP: open PRs and their review state, open issues and labels, branch protections. This is your standing work surface — note anything blocking a clean PR flow in the assessment.
