# Constitution — Developer Preset Addendum
...
This Cabinet runs the **developer preset**: the software product-kind kit —
the org ships a software / web / app product (Flavor B is a general product
org; this preset is its software kit, first of a family — e-commerce and
services kits are later siblings). The day-1 work surfaces are the product
repo (GitHub), the database (Neon, read-only by default), the deploy
pipeline (Vercel via REST + probes), and the live product itself
(Playwright browser verdicts).
...
The developer preset operates across these systems. Each has a distinct purpose:
...
## Developer Preset Quality Standards

Beyond the framework-level quality standards, the developer preset adds:
...
The developer preset uses capability-routed hooks. Default mapping (overridable in `cabinet/officer-capabilities.conf`):
