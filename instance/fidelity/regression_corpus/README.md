# Regression corpus store — frozen human corrections (flywheel §6.2)

This deployment's frozen-correction corpus (relocated from
`framework/fidelity/regression_corpus/` by egg plan R009, 2026-07-07 —
harvested Captain corrections are deployment data, so they live in the
instance layer; the mechanism stays in framework). Every Captain correction
(edit / skip / veto / undo / human-wrong verdict) on the consequence ledger is
frozen here as a replayable regression case, so the F1/eval cadence can gate
behavior changes on task-level non-regression:
...
- **Harvester:** `cabinet/scripts/build-regression-corpus.py` (thin CLI over
  `framework/fidelity/regression_corpus_lib.py`; scheduled daily via
  `com.cabinet.regression-corpus` — `cabinet/services.yml`). Reads the ledger
  read-only via `consequence.read_ledger()` (dedup, sim-quarantine,
  `CABINET_EVENT_LOG_DIR`). Idempotent + deterministic: re-runs append new
  cases only.
