"""flavor_a.acting — the Flavor-A (screenpipe) acting surface.

Re-homed from the former ``framework.acting.screenpipe_adapter`` (SRC-3 source-
adapter split): the screenpipe-coupled gather->draft front-end of the acting
lane. It calls the battle-tested screenpipe ``draft_lib`` / ``commitments_lib`` /
``product_ops_lib`` / ``context_lib`` in-process (the same cross-estate pattern
the fidelity adapter uses). NOTHING here sends: the present + dispatch deps are
wired separately and stay gated.

This module lives in the Flavor-A instance adapter package; framework CORE reaches
it ONLY through ``framework.sources.get_source()`` (the ``ScreenpipeSource``
methods delegate here). The PURE loop-plumbing (parse_dt / dedup / handled-sig /
lane_for) stays in ``framework.acting.lane_dedup`` and is imported below (an
instance -> framework import, which the layer boundary allows). Function bodies
are byte-identical to the originals.

py3.9-compatible runtime.
"""
...
# PURE loop-plumbing stays in framework; the screenpipe acting surface uses two of
# its helpers (instance -> framework import is allowed by the layer boundary).
from framework.acting.lane_dedup import parse_dt, _redis_host
